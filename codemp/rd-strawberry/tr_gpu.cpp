...
VkPresentModeKHR
PickBestPresentMode(const std::vector<VkPresentModeKHR>& presentModes)
{
...
VkInstance CreateInstance()
{
    ...
    Com_Printf("%d instance extensions will be used\n", requiredExtensionCount);
...
void CreateDescriptorSetLayouts(GpuContext& context)
{
    // single texture
    {
        ...
        if (vkCreateDescriptorSetLayout(
                ...
                &setLayoutCreateInfo,
                nullptr,
                &context.descriptorSetLayouts[DESCRIPTOR_SET_SINGLE_TEXTURE]) !=
            VK_SUCCESS)
        {
            Com_Error(
                ERR_FATAL,
...

    {
        ...
        if (vkCreateDescriptorSetLayout(
                ...
                &setLayoutCreateInfo,
                nullptr,
                &context.descriptorSetLayouts[DESCRIPTOR_SET_MULTI_TEXTURE]) !=
            VK_SUCCESS)
        {
            Com_Error(
                ERR_FATAL,
...
VkCullModeFlags GetVkCullMode(uint32_t stateBits2)
{
    ...
    {
    case GLS2_CULLMODE_FRONT:
        return VK_CULL_MODE_BACK_BIT;
    case GLS2_CULLMODE_BACK:
        return VK_CULL_MODE_FRONT_BIT;
    case GLS2_CULLMODE_NONE:
...
bool operator==(const RenderState& lhs, const RenderState& rhs)
{
    return (
        ...
        (lhs.stateBits == rhs.stateBits) && (lhs.stateBits2 == rhs.stateBits2));
}
...
void GpuContextInit(GpuContext& context)
{
    ...
    Com_Printf("Best physical device: %s\n", context.physicalDeviceName);
    ...
    if (!ri.VK_CreateWindowSurface(
            context.instance, (void**)&context.windowSurface))
    {
    ...
    for (int i = 0; i < queueFamilyCount; ++i)
    {
        ...
        vkGetPhysicalDeviceSurfaceSupportKHR(
            context.physicalDevice, i, context.windowSurface, &presentSupport);
    ...
    InitializeDeviceQueue(context.device, context.graphicsQueue);
    InitializeDeviceQueue(context.device, context.computeQueue);
    InitializeDeviceQueue(context.device, context.transferQueue);
    InitializeDeviceQueue(context.device, context.presentQueue);
    ...
    vkGetPhysicalDeviceSurfaceCapabilitiesKHR(
        context.physicalDevice, context.windowSurface, &surfaceCapabilities);

    uint32_t swapchainImageCount = surfaceCapabilities.minImageCount + 1;
    if (surfaceCapabilities.maxImageCount > 0)
    {
        swapchainImageCount =
            std::min(swapchainImageCount, surfaceCapabilities.maxImageCount);
    }
    ...
    Com_Printf("%d window surface formats available\n", surfaceFormatCount);
    ...
    Com_Printf("%d window present modes available\n", presentModeCount);
    ...
    const std::array<uint32_t, 2> swapchainQueueFamilyIndices = {
        context.graphicsQueue.queueFamily, context.presentQueue.queueFamily};

    if (context.graphicsQueue.queueFamily == context.presentQueue.queueFamily)
    {
    ...
    context.depthStencilFormat = PickDepthStencilFormat(context.physicalDevice);
    ...
    //
    {
        ...
        cmdPoolCreateInfo.queueFamilyIndex = context.transferQueue.queueFamily;
...

    {
        ...
        cmdPoolCreateInfo.flags =
            VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT;
        cmdPoolCreateInfo.queueFamilyIndex = context.graphicsQueue.queueFamily;
    ...
    context.swapchain.imagesInFlight.resize(
    ...
    for (uint32_t i = 0; i < swapchainImageCount; ++i)
    {
        ...
        std::array<VkImageView, 2> attachments = {
            resources.imageView, context.depthImageView};
...
void GpuContextShutdown(GpuContext& context)
{
    ...
    for (auto& swapchainResources : context.swapchain.resources)
    {
        vkDestroyDescriptorPool(
            context.device, swapchainResources.descriptorPool, nullptr);

        vkDestroyImageView(
            context.device, swapchainResources.imageView, nullptr);

        vkDestroyFramebuffer(
            context.device, swapchainResources.framebuffer, nullptr);
    }
    ...
    {
        vkDestroySemaphore(
            context.device, frameResources.imageAvailableSemaphore, nullptr);

        vkDestroySemaphore(
            context.device, frameResources.renderFinishedSemaphore, nullptr);

        vkDestroyFence(
            context.device, frameResources.frameExecutedFence, nullptr);
    }
...
VkShaderModule
GpuCreateShaderModuleFromFile(GpuContext& context, const char* filePath)
{
    void* code = nullptr;
    ...
    {
        Com_Error(
            ERR_FATAL, "Failed to load shader module from file '%s'", filePath);
    }
    ...
    GpuContext& context, const RenderState& renderState)
{
    ...
    viCreateInfo.vertexAttributeDescriptionCount =
    ...
    rsCreateInfo.polygonMode = (renderState.stateBits & GLS_POLYMODE_LINE)
    ...
    dsCreateInfo.depthCompareOp = (renderState.stateBits & GLS_DEPTHFUNC_EQUAL)
                                      ? VK_COMPARE_OP_EQUAL
...
VkDescriptorSet GpuAllocateDescriptorSet(
    GpuContext& context, DescriptorSetId descriptorSetLayoutId)
{
    ...
    if (vkAllocateDescriptorSets(
            gpuContext.device, &descriptorSetAllocateInfo, &descriptorSet) !=
        VK_SUCCESS)
    {
...
VkDescriptorSet
GpuCreateDescriptorSet(GpuContext& context, const image_t* image)
{
...
static TransientBufferEntry* FindTransientBuffer(
    TransientResources& transientBuffers,
    size_t size,
...
TransientBuffer*
GpuGetTransientVertexBuffer(TransientResources& transientBuffers, size_t size)
{
    return GetTransientBuffer(
        transientBuffers, size, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT);
}

TransientBuffer*
GpuGetTransientIndexBuffer(TransientResources& transientBuffers, size_t size)
{
    return GetTransientBuffer(
