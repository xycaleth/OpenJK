// Filename:-	model.cpp
//
// non-format specific model routines entry point, calls format-specific code
// from within here
//
// ( This is the nice clean gateway module into all the evil crap I have to call
...
static int
Model_MultiSeq_GetSeqHint(ModelContainer_t* pContainer, bool bPrimary);
static void Model_MultiSeq_SetSeqHint(
    ModelContainer_t* pContainer, bool bPrimary, int iHint);
static bool Model_MultiSeq_EnsureSeqHintLegal(
    ModelContainer_t* pContainer, int iFrame, bool bPrimary);

#define sERROR_MODEL_NOT_LOADED                                                \
    "Error: Model not loaded, you shouldn't get here! -Ste"
#define sERROR_CONTAINER_NOT_FOUND                                             \
    "Error: Could not resolve model handle to container ptr, you shouldn't "   \
    "get here! -Ste"
#define sSECONDARY_ANIM_STATS_STRING                                           \
    "(Secondary anim)" // try and keep this fairly short, since it occupies
                       // roughly the same space as "bolt: <boltname>"
...
bool gbRenderInhibit =
    false; // MUST stay in this state except when loading a model
...
static void
R_ModelContainer_CallBack_InfoText(ModelContainer_t* pContainer, void* pvData);
typedef struct // simple struct for passing text data to callback functions
               // during info printing (ZEROMEM'd)
{
    int iTextY;
    int iTextX;
    int iPrevX;
...
// returns NULL if not attached to anything, else name of tag-surface or
// boltpoint
//
static const char*
Stats_GetParentAttachmentPointString(ModelContainer_t* pContainer)
{
    const char* psAttachedVia =
        (!pContainer->pBoneBolt_ParentContainer)
            ? ((!pContainer->pSurfaceBolt_ParentContainer)
                   ? NULL
                   : pContainer->pSurfaceBolt_ParentContainer
                         ->tSurfaceBolt_BoltPoints
                             [pContainer->iSurfaceBolt_ParentBoltIndex]
                         .sAttachName.c_str())
            : pContainer->pBoneBolt_ParentContainer
                  ->tBoneBolt_BoltPoints[pContainer->iBoneBolt_ParentBoltIndex]
                  .sAttachName.c_str();
...
{
    const char* psAttachedVia =
        Stats_GetParentAttachmentPointString(pContainer);
    const char* psAttachmentString =
        va("%s", psAttachedVia ? va("(bolt: \"%s\")", psAttachedVia) : "");

    return psAttachmentString;
}

static void R_ModelContainer_Apply_Actual(
    ModelContainer_t* pContainer,
    void (*pFunction)(ModelContainer_t* pContainer, void* pvData), void* pvData,
    bool bFromBottomUp)
{
    int iBoltPoint = 0;
    ...
    //
    for (iBoltPoint = 0; iBoltPoint < pContainer->iBoneBolt_MaxBoltPoints;
         iBoltPoint++)
    {
        BoltPoint_t* pBoltPoint = &pContainer->tBoneBolt_BoltPoints[iBoltPoint];

        for (std::size_t iBoltOn = 0;
             iBoltOn < pBoltPoint->vBoltedContainers.size();
             iBoltOn++)
        {
            R_ModelContainer_Apply(
                &pBoltPoint->vBoltedContainers[iBoltOn], pFunction, pvData);
        }
    }

    // process this container's surface bolts...
    //
    for (iBoltPoint = 0; iBoltPoint < pContainer->iSurfaceBolt_MaxBoltPoints;
         iBoltPoint++)
    {
        BoltPoint_t* pBoltPoint =
            &pContainer->tSurfaceBolt_BoltPoints[iBoltPoint];
...
// same as above, but calls from bottom of recursion tree to top, so can destroy
// ptrs to lower elements during freeup...
//
static void R_ModelContainer_ApplyFromBottomUp(
    ModelContainer_t* pContainer,
    void (*pFunction)(ModelContainer_t* pContainer, void* pvData),
    void* pvData = NULL);
static void R_ModelContainer_ApplyFromBottomUp(
    ModelContainer_t* pContainer,
    void (*pFunction)(ModelContainer_t* pContainer, void* pvData), void* pvData)
...
// set the supplied container to be empty...  (note that because of stl, 99% of
// this works for either init or dealloc)
//
static void ModelContainer_Clear(
    ModelContainer_t* pContainer,
    void* pvData =
        NULL); // last field provided for R_ModelContainer_Apply() only
static void ModelContainer_Clear(ModelContainer_t* pContainer, void* pvData)
{
    ...
    pContainer->iBoneNum_SecondaryStart =
        -1; // default, meaning "ignore", else bone num to stop primary
            // animation on, and begin secondary
    ...
    pContainer->iSeqMultiLock_Primary_SeqHint =
        0; // not really important what number is picked
    ...
    // special linkage stuff (this can all safely be cleared since during freeup
    // we're called recursively backwards)...
    //
    if (pContainer->tBoneBolt_BoltPoints.size() ||
        pContainer->tSurfaceBolt_BoltPoints.size())
    {
        AppVars.iTotalContainers--; // we're calling this to free stuff, not to
                                    // init it
    }
    //
    ...
    { // bone bolts...
        ...
        pContainer->iBoneBolt_ParentBoltIndex =
            -1; // .. if we're the root model, seems a reasonable default (as is
                // NULL on line above)
        pContainer->iBoneBolt_MaxBoltPoints =
            -1; // similar to modtype_bad, should never exist when init code
                // called after this
    ...
    { // surface bolts...
        ...
        pContainer->iSurfaceBolt_ParentBoltIndex =
            -1; // .. if we're the root model, seems a reasonable default (as is
                // NULL on line above)
        pContainer->iSurfaceBolt_MaxBoltPoints =
            -1; // similar to modtype_bad, should never exist when init code
                // called after this
...
// this is stuff that gets cleared during both AppVars_OnceOnlyInit() and
// Model_Delete()...
//
static void AppVars_Delete(void)
{
    //	int iNumBolts = 0;
    R_ModelContainer_ApplyFromBottomUp(
        &AppVars.Container, ModelContainer_Clear); //, &iNumBolts);
...
#define OUTDOUBLE(blah) strOut += va("%s:%f\n", #blah, AppVars.blah);
        ...
        const char* psIdealName =
            va("%s.ideal",
               Filename_WithoutExt(AppVars.strLoadedModelPath.c_str()));
        ...
        {
            ErrorBox(
                va("Unable to write \"%s\"!, write protected?", psIdealName));
        }
    }
    else
    {
...
void AppVars_ReadIdeal(void)
{
    if (!AppVars.strLoadedModelPath.empty())
    {
        ...
        if (fHandle)
        {
            ...
            {
                // deal with CR stuff manually, in case file was edited by hand
                // and last line doesn't have one...
                //
                if (strchr(sLine, '\n'))
                    *strchr(sLine, '\n') = '\0';
            ...
            if (Gallery_Active())
            {
                // this won't actually put up a box if the gallery is active,
                // but it will add it to the
                //	overall report file so they'll know about it...
                //
                // (I may offer the ability to use these, but can't be bothered
                // hassling them with extra
                //	Yes/No queries at the moment)
                //
                WarningBox(
                    va("Ignoring settings file \"%s\" during gallery snapshots",
                       psIdealName));
                return;
            }
#endif
...
// this deletes the primary model, all bolted models, and the low-level model
// cache
//
...
const char*
Model_GetSupportedTypesFilter(bool bScriptsEtcAlsoAllowed /* = false */)
{
...
// findme: All code that uses cut/paste from other projects should go through
// here, since it allows the whole
//			code-exit mechanism that they use to be trapped
//properly...
//
...
static void
ModelContainer_CallBack_HandleCheck(ModelContainer_t* pContainer, void* pvData)
{
...
ModelContainer_t* ModelContainer_FindFromModelHandle(ModelHandle_t hModel)
{
    ...
    R_ModelContainer_Apply(
        &AppVars.Container, ModelContainer_CallBack_HandleCheck, &hModel);
...
// read in a model using main engine code, then parse any extra stuff that this
// modview apps wants to know about...
//
// note that this doesn't know or care whether it's the parent container or a
// bolt on, and neither should it.
    ...
    if (hModel)
    {
        ...
        try
        {
            if ((modtype = RE_GetModelType(hModel)) == MOD_MDXM)
            {
                ...
                // trap_G2_Set_Bone_Anim(ent->ghoulmodel, ent->s.blist,
                // "model_root", 0, 9, BONE_ANIM_OVERRIDE_LOOP, 0.1f);
            }
        }
        ...
        // now do any of my post-process stuff... (which doesn't need try-catch
        ...
        if (hModel)
        {
            ...
            default:
                ...
                bModelOk = false;
                ErrorBox(
                    va("The model \"%s\" is valid, but ModView doesn't fully "
                       "support this type at present",
                       psLocalFilename));
                break;
            }
            ...
            if (bModelOk)
            {
                // the above switch-case should have filled in these
                // per-format...
                //
                assert(
                    pContainer->iBoneBolt_MaxBoltPoints !=
                    -1); // check that deliberate illegal default is overwritten
                assert(
                    pContainer->iSurfaceBolt_MaxBoltPoints !=
                    -1); // check that deliberate illegal default is overwritten
                ...
                // if failed to read any sequence files then make a default
                // one...
                //
                if (!pContainer->SequenceList.size())
                {
                    pContainer->SequenceList.push_back(
                        *Sequence_CreateDefault(pContainer->iNumFrames));
                }

                // default bolton stuff (ensure that matrix mem initialised, and
                // bolton array resized correctly...)
                //
                // bone bolts...
                //
                pContainer->tBoneBolt_BoltPoints.resize(
                    pContainer->iBoneBolt_MaxBoltPoints);
                for (iBoltPoint = 0;
                     iBoltPoint < pContainer->iBoneBolt_MaxBoltPoints;
                     iBoltPoint++)
                {
                    BoltPoint_t* pBoltPoint =
                        &pContainer->tBoneBolt_BoltPoints[iBoltPoint];

                    pBoltPoint->vMatricesPerFrame.resize(
                        pContainer->iNumFrames);
                    pBoltPoint->sAttachName =
                        pContainer->pModelGetBoneBoltNameFunction(
                            pContainer->hModel, iBoltPoint);
                    pBoltPoint->vBoltedContainers
                        .clear(); // probably not nec., but wtf?
                }

                //
                // surface bolts...
                //
                pContainer->tSurfaceBolt_BoltPoints.resize(
                    pContainer->iSurfaceBolt_MaxBoltPoints);
                for (iBoltPoint = 0;
                     iBoltPoint < pContainer->iSurfaceBolt_MaxBoltPoints;
                     iBoltPoint++)
                {
                    BoltPoint_t* pBoltPoint =
                        &pContainer->tSurfaceBolt_BoltPoints[iBoltPoint];
                    ...
                    pBoltPoint->vMatricesPerFrame.resize(
                        pContainer->iNumFrames);
                    pBoltPoint->sAttachName =
                        pContainer->pModelGetSurfaceBoltNameFunction(
                            pContainer->hModel, iBoltPoint);
                    pBoltPoint->vBoltedContainers.clear();
                }

                // finally, we can do stuff like skin file code that can popup
                // GetYesNo boxes, which cause grief
                //	if they happen before the bolt stuff above has
                //occured...
                //
                switch (modtype)
                {
                case MOD_MDXM:
                    ...
                    // only one of these will be valid at once, so no need to
                    // check...
                    //
...
{
    /*if (gModViewTreeViewhandle)	// will be valid unless this is called from
    app exit
    {
...
bool ModelTree_SetItemText(HTREEITEM hTreeItem, const char* psText)
{
    ...
    {
            return
    !!gModViewTreeViewhandle->GetTreeCtrl().SetItemText(hTreeItem, psText);
    }
...
// param 'bPure' should be TRUE if you want to strip stuff like "////" from
// "///////// surfacename",
//	and return the original un-decorated text by querying the model directly
//if possible...
//
...
const char* ModelTree_GetItemText(HTREEITEM hTreeItem, bool bPure /* = false */)
{
    ...
    app exit
    {
            if (bPure)
            {
                    // let's see if this is a treeitem type that can return pure
    text...
                    //
                    TreeItemData_t	TreeItemData;
                                                    TreeItemData.uiData =
    ModelTree_GetItemData(hTreeItem);

                    if (TreeItemData.iItemType == TREEITEMTYPE_GLM_SURFACE
                            ||
                            TreeItemData.iItemType ==
    TREEITEMTYPE_GLM_TAGSURFACE
                            )
                    {
                            return GLMModel_GetSurfaceName(
    TreeItemData.iModelHandle, TreeItemData.iItemNumber );
                    }
            }
            ...
            static CString	string;
                                            string =
    gModViewTreeViewhandle->GetTreeCtrl().GetItemText(hTreeItem); return (const
    char *) string;
    }
...
//
static HTREEITEM R_ModelTree_FindItemWithThisData(
    HTREEITEM hTreeItem, UINT32 uiData2Match, int* piItemsScanned = NULL);
static HTREEITEM R_ModelTree_FindItemWithThisData(
    HTREEITEM hTreeItem, UINT32 uiData2Match, int* piItemsScanned /*=NULL*/)
{
    if (!hTreeItem)
    ...
    if (hTreeItem)
    {
        ...
        //		OutputDebugString(va("Scanning item %X
        //(%s)\n",hTreeItem,psText));
        ...
        if (hTreeItem_Child)
        {
            HTREEITEM hTreeItemFound = R_ModelTree_FindItemWithThisData(
                hTreeItem_Child, uiData2Match, piItemsScanned);
            if (hTreeItemFound)
        ...
        {
            HTREEITEM hTreeItemFound = R_ModelTree_FindItemWithThisData(
                hTreeItem_Sibling, uiData2Match, piItemsScanned);
            if (hTreeItemFound)
                return hTreeItemFound;
        }

        // this treeitem isnt a match, and neither are its siblings or children,
        // so...
        //
        return NULL;
    }
...
int ModelTree_GetChildCount(HTREEITEM hTreeItem)
{
    ...
    if (gModViewTreeViewhandle) // will be valid unless this is called from app
                                // exit
    {
        if (gModViewTreeViewhandle->GetTreeCtrl().ItemHasChildren(hTreeItem))
        {
            hTreeItem =
                gModViewTreeViewhandle->GetTreeCtrl().GetChildItem(hTreeItem);
            R_ModelTree_FindItemWithThisData(
                hTreeItem,
                0xDEADDEAD,
                &iChildCount); // massive-function abuse here! :-)
        }
    }
    * /
...
bool ModelTree_ItemHasChildren(HTREEITEM hTreeItem)
{
    ...
    {
            return
    !!gModViewTreeViewhandle->GetTreeCtrl().ItemHasChildren(hTreeItem);
    }
...
HTREEITEM ModelTree_GetNextSiblingItem(HTREEITEM hTreeItem)
{
    ...
    {
            return
    gModViewTreeViewhandle->GetTreeCtrl().GetNextSiblingItem(hTreeItem);
    }
...
bool ModelTree_DeleteItem(HTREEITEM hTreeItem)
{
    ...
    {
            return
    !!gModViewTreeViewhandle->GetTreeCtrl().DeleteItem(hTreeItem);
    }
...
// this should only be called when you know it's a GLM model for the moment...
// (put in for Keith's remote access)
//
HTREEITEM ModelTree_GetRootSurface(ModelHandle_t hModel)
{
    /*if (gModViewTreeViewhandle && Model_Loaded(hModel))	// will be valid
    unless this is called from app exit
    {
            TreeItemData_t	TreeItemData = {0};
                                            TreeItemData.iItemType		=
    TREEITEMTYPE_SURFACEHEADER; TreeItemData.iModelHandle	= hModel;

            HTREEITEM hTreeItem_SurfaceHeader =
    R_ModelTree_FindItemWithThisData(NULL, TreeItemData.uiData); if
    (hTreeItem_SurfaceHeader)
            {
...
HTREEITEM ModelTree_GetRootBone(ModelHandle_t hModel)
{
    ...
    unless this is called from app exit
    {
            TreeItemData_t	TreeItemData = {0};
                                            TreeItemData.iItemType		=
    TREEITEMTYPE_BONEHEADER; TreeItemData.iModelHandle	= hModel;

            HTREEITEM hTreeItem_BoneHeader =
    R_ModelTree_FindItemWithThisData(NULL, TreeItemData.uiData); if
    (hTreeItem_BoneHeader)
            {
...
void ModelTree_InsertSequences(
    ModelHandle_t hModel, HTREEITEM hTreeItem_Sequences)
{
...
void ModelTree_InsertSequences(
    ModelContainer_t* pContainer, HTREEITEM hTreeItem_Sequences)
{
    /*CWaitCursor wait;
    ...
    //
    HTREEITEM hTreeItemChild =
    gModViewTreeViewhandle->GetTreeCtrl().GetChildItem(hTreeItem_Sequences); if
    (hTreeItemChild)
    {
            while (hTreeItemChild)
            {
                    HTREEITEM hNext =
    gModViewTreeViewhandle->GetTreeCtrl().GetNextSiblingItem(hTreeItemChild);
    ...
    //
    for (iSequenceIndex = 0; iSequenceIndex<pContainer->SequenceList.size();
    iSequenceIndex++)
    {
    ...
    // sort 'em...
    //
    qsort(&Sequences[0], Sequences.size(), sizeof(Sequence_t*),
    AppVars.bSortSequencesByAlpha ? SortSequenceBy_Alpha :
    SortSequenceBy_FrameNum);
    ...
    for (int i=0; i<Sequences.size(); i++)
    {
            ...
            {
                    TreeItemData_t	TreeItemData				=
    {0}; TreeItemData.iItemType		= TREEITEMTYPE_SEQUENCE;
                                                    TreeItemData.iItemNumber
    = iSequenceIndex; TreeItemData.iModelHandle	= pContainer->hModel;

                    HTREEITEM htiThis = ModelTree_InsertItem(
    Sequence_CreateTreeName(pSequence),	// LPCTSTR psName, hTreeItem_Sequences,
    // HTREEITEM hParent TreeItemData.uiData
    //	TREEITEMTYPE_GLM_SURFACE | iThisSurfaceIndex	// UINT32 uiUserData
                                                                                                    );
            }
            else
            {
                    assert(0 && "ModelTree_InsertSequences(): Unknown sequence
    ptr!");
            }
    }*/
}
...
// hParent can be NULL...
//
HTREEITEM ModelTree_InsertItem(
    LPCTSTR psName, HTREEITEM hParent, UINT32 uiUserData /* = NULL */,
    HTREEITEM hInsertAfter /* = TVI_LAST */)
{
    /*if (gModViewTreeViewhandle)	// will be valid unless this is called from
    app exit
    {
            return gModViewTreeViewhandle->InsertItem(psName, hParent,
    uiUserData, hInsertAfter);
    }
...
// ensure that there's a "BoltOns" header in a container, and if not, add one
// and add it to the tree also.
//
static bool ModelContainer_EnsureBoltOnHeader(ModelContainer_t* pContainer)
{
    ...
    pContainer->hTreeItem_BoltOns = ModelTree_InsertItem(
        "BoltOns", pContainer->hTreeItem_ModelName, TreeItemData.uiData);
...
static bool Model_LoadPrimaryImpl(const char* psFullPathedFilename)
{
    ...
    // I'll check for file existance first, to avoid deleting current model
    // until a valid replacement is ready...
    //
...
void Model_ApplyEthnicSkin(
    ModelHandle_t hModel, const char* psSkin, const char* psEthnic,
...
void Model_ApplySkinShaderVariant(
    ModelHandle_t hModel, const char* psSkin, const char* psEthnic,
    const char* psMaterial, int iVariant)
{
    ...
    if (pContainer)
    {
        Skins_ApplySkinShaderVariant(
            pContainer, psSkin, psEthnic, psMaterial, iVariant);
    }
    else
    {
        assert(0);
...
// deletes whatever's bolted to the supplied bolt number of the suppplied
// model...
//
bool Model_DeleteBoltOn(
    ModelHandle_t hModel, int iBoltPointToDelete, bool bBoltIsBone,
    int iBoltOnAtBoltPoint)
{
    bool bReturn = false;
    ...
    if (pContainer)
    {
        return Model_DeleteBoltOn(
            pContainer, iBoltPointToDelete, bBoltIsBone, iBoltOnAtBoltPoint);
    }
    else
    {
        assert(0);
...
// deletes <iBoltOnAtBoltPoint / all(-1)> from <iBoltPointToDelete> on
// <pContainer>
//
bool Model_DeleteBoltOn(
    ModelContainer_t* pContainer, int iBoltPointToDelete, bool bBoltIsBone,
    int iBoltOnAtBoltPoint)
{
    bool bReturn = false;

    std::vector<BoltPoint_t>& vBoltPoints =
        bBoltIsBone ? pContainer->tBoneBolt_BoltPoints
                    : pContainer->tSurfaceBolt_BoltPoints;
    int& iMaxBoltPoints = bBoltIsBone ? pContainer->iBoneBolt_MaxBoltPoints
                                      : pContainer->iSurfaceBolt_MaxBoltPoints;

    if (iBoltPointToDelete < (bBoltIsBone
                                  ? pContainer->iBoneBolt_MaxBoltPoints
                                  : pContainer->iSurfaceBolt_MaxBoltPoints))
    {
        if (static_cast<std::size_t>(iBoltPointToDelete) <
            (bBoltIsBone ? pContainer->tBoneBolt_BoltPoints.size()
                         : pContainer->tSurfaceBolt_BoltPoints.size()))
        {
            BoltPoint_t* pBoltOn =
                bBoltIsBone
                    ? &pContainer->tBoneBolt_BoltPoints[iBoltPointToDelete]
                    : &pContainer->tSurfaceBolt_BoltPoints[iBoltPointToDelete];
            // stl-type limits, ie [Begin, End)...
            //
            int iBoltOnBegin =
                (iBoltOnAtBoltPoint == -1) ? 0 : iBoltOnAtBoltPoint;
            int iBoltOnEnd = (iBoltOnAtBoltPoint == -1)
                                 ? pBoltOn->vBoltedContainers.size()
                                 : iBoltOnAtBoltPoint + 1;

            for (int iBoltOn = iBoltOnBegin; iBoltOn != iBoltOnEnd; iBoltOn++)
            {
                ModelContainer_t* pBoltedContainer =
                    &pBoltOn->vBoltedContainers[iBoltOn];

                // delete all tree items from the bolt downwards... (including
                // models bolted to the bolt)
                //
#ifdef USE_MFC
                ModelTree_DeleteItem(pBoltedContainer->hTreeItem_ModelName);
#endif

                // delete underlying containers... (including models bolted to
                // the bolt)
                //
                R_ModelContainer_ApplyFromBottomUp(
                    pBoltedContainer, ModelContainer_Clear);

                /*				// delete the bolt itself and mark it
                   as empty...
                                                //
                                                if (bBoltIsBone)
                                                {
                                                        delete
                   (pContainer->tBoneBolt_BoltPoints[iBoltPointToDelete].pBoltedContainer);
                                                                        pContainer->tBoneBolt_BoltPoints[iBoltPointToDelete].pBoltedContainer
                   = NULL;
                                                }
                                                else
                                                {
                                                        delete
                   (pContainer->tSurfaceBolt_BoltPoints[iBoltPointToDelete].pBoltedContainer);
                                                                        pContainer->tSurfaceBolt_BoltPoints[iBoltPointToDelete].pBoltedContainer
                   = NULL;
                                                }
                */
            }
            pBoltOn->vBoltedContainers.erase(
                pBoltOn->vBoltedContainers.begin() + iBoltOnBegin,
                pBoltOn->vBoltedContainers.begin() + iBoltOnEnd);

            // finally, for neatness, check if our BoltOns treeitem is now empty
            // (no children), and delete it if so.
        ...
        {
            ErrorBox(
                va("Model_DeleteBoltOn(): %s bolt index %d is legal (max %d), "
                   "but container only has %d entries!!! (Tell me! -Ste)",
    ...
    {
        ErrorBox(
            va("Model_DeleteBoltOn(): Illegal %s bolt index %d (max = %d)!",
               bBoltIsBone ? "bone" : "surface",
               iBoltPointToDelete,
               bBoltIsBone ? pContainer->iBoneBolt_MaxBoltPoints
                           : pContainer->iSurfaceBolt_MaxBoltPoints));
    }
...
//
static int Model_GetBoltOnNumber(
    ModelContainer_t* pContainer, ModelContainer_t* pParentContainer,
    int iParentBoltIndex, bool bBoltIsBone)
{
    std::vector<BoltPoint_t>& vParentBoltPoints =
        bBoltIsBone ? pParentContainer->tBoneBolt_BoltPoints
                    : pParentContainer->tSurfaceBolt_BoltPoints;
    ...
    if (static_cast<std::size_t>(iParentBoltIndex) < vParentBoltPoints.size())
    {
        ...
        for (std::size_t iBoltOn = 0;
             iBoltOn < pBoltOn->vBoltedContainers.size();
             iBoltOn++)
        {
    ...
    {
        ErrorBox(
            va("Model_GetBoltOnNumber(): Error, illegal %s bolt index %d (max "
               "= %d)!  (Tell me! - Ste)\n",
               (bBoltIsBone ? "bone" : "surface"),
               iParentBoltIndex,
               vParentBoltPoints.size()));
    }

    return -1;
}

// deletes the supplied container of a bolted model from whatever its bolted
// to...
//
...
bool Model_DeleteBoltOn(ModelContainer_t* pContainer)
{
    ...
    {
        int iBoltOnNumber = Model_GetBoltOnNumber(
            pContainer,
            pContainer->pBoneBolt_ParentContainer,
            pContainer->iBoneBolt_ParentBoltIndex,
            true);
...
bool Model_DeleteBoltOn(ModelHandle_t hModelBoltOn)
{
    ...
    ModelContainer_t* pContainer =
        ModelContainer_FindFromModelHandle(hModelBoltOn);
...
// used for UI update check...  (also used in renderer now, to work out whether
// or not to xform a g2 tag surface
//
int Model_CountItemsBoltedHere(
    ModelHandle_t hModel, int iBoltIndex, bool bBoltIsBone)
{
    ModelContainer_t* pContainer = ModelContainer_FindFromModelHandle(hModel);
    ...
    if (pContainer)
    {
        if (bBoltIsBone)
        {
            ...
            {
                return pContainer->tBoneBolt_BoltPoints[iBoltIndex]
                    .vBoltedContainers.size();
            }
        }
        else
        {
            ...
            {
                return pContainer->tSurfaceBolt_BoltPoints[iBoltIndex]
                    .vBoltedContainers.size();
            }
        }
    }
...
bool Model_LoadBoltOn(
    const char* psFullPathedFilename, ModelHandle_t hModel,
    const char* psBoltName, bool bBoltIsBone, bool bBoltReplacesAllExisting)
{
    ...
    return Model_LoadBoltOn(
        psFullPathedFilename,
        hModel,
        iBoltIndex,
        bBoltIsBone,
        bBoltReplacesAllExisting);
}

static bool _Actual_Model_LoadBoltOn(
    const char* psFullPathedFilename, ModelHandle_t hModel, int iBoltIndex,
    bool bBoltIsBone, bool bBoltReplacesAllExisting)
{
    if (!hModel)
    {
    ...
    // ok, any errors from here on in will delete the primary model if they
    // occur...
    //
    bool bReturn = false;
    ...
    if (pContainer)
    {
            ...
            iBoltIndex >= 0)
        {
            ...
            // ModelContainer_RegisterModel(strLocalFilename, pContainer_BoltOn,
            // pContainer->hTreeItem_BoltOns);

            // note that we do this code to link everything in whether or not we
            // succeeded in registering the model
            //	so that the Model_Delete code catches everything, ie we don't
            //have any orphaned pointers
            if (bBoltIsBone)
            {
                pContainer->tBoneBolt_BoltPoints[iBoltIndex]
                    .vBoltedContainers.push_back(*pContainer_BoltOn);
            }
            else
            {
                pContainer->tSurfaceBolt_BoltPoints[iBoltIndex]
                    .vBoltedContainers.push_back(*pContainer_BoltOn);
            }

            std::vector<ModelContainer_t>& vBoltedContainers =
                bBoltIsBone ? pContainer->tBoneBolt_BoltPoints[iBoltIndex]
                                  .vBoltedContainers
                            : pContainer->tSurfaceBolt_BoltPoints[iBoltIndex]
                                  .vBoltedContainers;

            ModelContainer_t* pNewContainerLocation =
                &vBoltedContainers[vBoltedContainers.size() - 1];
            ModelHandle_t hModel = ModelContainer_RegisterModel(
                strLocalFilename.c_str(), pNewContainerLocation);
            ...
            {
                pNewContainerLocation->pSurfaceBolt_ParentContainer =
                    pContainer;
                pNewContainerLocation->iSurfaceBolt_ParentBoltIndex =
                    iBoltIndex;
            }
        ...
        {
            ErrorBox(
                va("Illegal %s bolt index %d specified, max = %d",
                   (bBoltIsBone ? "bone" : "surface"),
                   iBoltIndex,
                   bBoltIsBone ? pContainer->iBoneBolt_MaxBoltPoints
                               : pContainer->iSurfaceBolt_MaxBoltPoints));
    ...
    ModelList_ForceRedraw(); // update window, needed for when called by
                             // external client progs
...
{
    gbRenderInhibit = true;
    bool b = _Actual_Model_LoadBoltOn(
        psFullPathedFilename,
        hModel,
        iBoltIndex,
        bBoltIsBone,
...
const char* Model_GLMSurfaceInfo(
    ModelHandle_t hModel, int iSurfaceIndex, bool bShortVersionForTag)
{
...
bool Model_SurfaceContainsBoneReference(
    ModelHandle_t hModel, int iLODNumber, int iSurfaceNumber, int iBoneNumber)
{
    // fixme: at some point this'll need to be non-GLM specific, but for now...
    //
    if (Model_Loaded(hModel))
    {
        return GLMModel_SurfaceContainsBoneReference(
            hModel, iLODNumber, iSurfaceNumber, iBoneNumber);
    }
...
const char*
Model_GetSurfaceName(ModelContainer_t* pContainer, int iSurfaceIndex)
{
    if (pContainer->pModelGetSurfaceNameFunction)
    {
        return pContainer->pModelGetSurfaceNameFunction(
            pContainer->hModel, iSurfaceIndex);
    }
...
// this function isn't terribly fast, so try not to call it somewhere speed
// dependant...
//
...
//
int Model_GetBoltIndex(
    ModelHandle_t hModel, const char* psBoltName, bool bBoltIsBone)
{
    ModelContainer_t* pContainer = ModelContainer_FindFromModelHandle(hModel);
    if (pContainer)
    {
...
//
int Model_GetBoltIndex(
    ModelContainer_t* pContainer, const char* psBoltName,
    bool bBoltIsBone) // semi-recursive now
{
        ...
        pContainer->pModelGetSurfaceBoltNameFunction)
    {
        ...
        //
        for (int i = 0;
             i < (bBoltIsBone ? pContainer->iBoneBolt_MaxBoltPoints
                              : pContainer->iSurfaceBolt_MaxBoltPoints);
             i++)
        {
            if (!Q_stricmp(
                    Model_GetBoltName(pContainer, i, bBoltIsBone), psBoltName))
                return i;
        }
        ...
        if (bBoltIsBone)
        {
            ...
            if (!bAlreadyHere)
            {
                for (MappedString_t::iterator it = pContainer->Aliases.begin();
                     it != pContainer->Aliases.end();
                     ++it)
                {
                    ...
                    if (!Q_stricmp(strAliasName.c_str(), psBoltName))
                    {
                        ...
                        bAlreadyHere = true;
                        int iAnswer = Model_GetBoltIndex(
                            pContainer, strRealName.c_str(), bBoltIsBone);
                        bAlreadyHere = false;
                        return iAnswer;
                    }
                }
        ...
        ErrorBox(
            va("Model_GetBoltIndex(): Unable to find bolt called \"%s\"!",
               psBoltName));
        return -1;
    }

    ErrorBox(
        "Model_GetBoltIndex(): need either a ->pModelGetBoneBoltNameFunction "
        "or ->pModelGetSurfaceBoltNameFunction!");
    return -1;
}

const char*
Model_GetBoltName(ModelHandle_t hModel, int iBoltIndex, bool bBoltIsBone)
{
    ModelContainer_t* pContainer = ModelContainer_FindFromModelHandle(hModel);
    if (pContainer)
    {
...
const char* Model_GetBoltName(
    ModelContainer_t* pContainer, int iBoltIndex, bool bBoltIsBone)
{
    if (bBoltIsBone)
    {
        if (pContainer->pModelGetBoneBoltNameFunction)
        {
            return pContainer->pModelGetBoneBoltNameFunction(
                pContainer->hModel, iBoltIndex);
        }
    }
    else
    {
        ...
        {
            return pContainer->pModelGetSurfaceBoltNameFunction(
                pContainer->hModel, iBoltIndex);
        }
    }
...
// this returns the full disk path of the primary model... (mainly only used for
// new-file browsing)
//
const char* Model_GetFullPrimaryFilename(void)
{
    ...
    // only usually gets here on first File-Open if not launched via
    // file-association...
    //
    return "";
}
...
bool Model_GLMSurface_SetStatus(
    ModelHandle_t hModel, int iSurfaceIndex, SurfaceOnOff_t eStatus)
{
    if (Model_Loaded(hModel))
    {
...
bool Model_GLMSurface_SetStatus(
    ModelHandle_t hModel, const char* psSurfaceName, SurfaceOnOff_t eStatus)
{
    if (Model_Loaded(hModel))
    {
        ModelContainer_t* pContainer =
            ModelContainer_FindFromModelHandle(hModel);
        if (pContainer)
        {
            // convert surface name to index...
            //
            for (int iSurface = 0; iSurface < pContainer->iNumSurfaces;
                 iSurface++)
            {
                if (!Q_stricmp(
                        psSurfaceName,
                        pContainer->pModelGetSurfaceNameFunction(
                            pContainer->hModel, iSurface)))
                {
                    return Model_GLMSurface_SetStatus(
                        pContainer->hModel, iSurface, eStatus);
                }
            }
        }
        WarningBox(
            va("Model_GLMSurface_SetStatus():\nSurface \"%s\" not found!\n\n ( "
               "Model: \"%s\" )",
               psSurfaceName,
               pContainer->sLocalPathName));
        return false;
...
SurfaceOnOff_t
Model_GLMSurface_GetStatus(ModelHandle_t hModel, int iSurfaceIndex)
{
    if (Model_Loaded(hModel))
    {
...
// subroutinised so there's common logic for the 2 functions below this
// (important!)...
//
static bool Model_SurfaceIsDifferentFromDefault(
    ModelContainer_t* pContainer, SurfaceOnOff_t eStatusToCheck, int iSurface)
{
    SurfaceOnOff_t eThisStatus =
        Model_GLMSurface_GetStatus(pContainer->hModel, iSurface);
    ...
    {
        // this surface's status matches the query, now is it naturally this
        // way?
        //
        const char* psSurfaceName = Model_GetSurfaceName(pContainer, iSurface);

        if (psSurfaceName) // problems if we don't have this of course, but
                           // errormessaged already
        {
            bool bSurfaceNameIncludesOFF =
                !Q_stricmp("_off", &psSurfaceName[strlen(psSurfaceName) - 4]);
            ...
            case SURF_NO_DESCENDANTS:
                ...
                // no surface will ever have this set initially, the loader can
                // only set ON or OFF...
                //
                return true;
...
// script-query function, asks how many surfaces are set to 'eStatus' by the
// user, in other words how many
//	are different from their default state...
//
int Model_GetNumSurfacesDifferentFromDefault(
    ModelContainer_t* pContainer, SurfaceOnOff_t eStatus)
{
...
const char* Model_GetSurfaceDifferentFromDefault(
    ModelContainer_t* pContainer, SurfaceOnOff_t eStatus, int iSurfaceIndex)
{
    for (int iSurface = 0; iSurface < pContainer->iNumSurfaces; iSurface++)
    {
        if (Model_SurfaceIsDifferentFromDefault(pContainer, eStatus, iSurface))
...
void App_OnceOnly(void)
{
    ...
    OnceOnlyCrap(); // init some rubbish that cut/paste code wants for other
                    // formats

    TextureList_OnceOnlyInit();
}

ModelHandle_t Model_GetPrimaryHandle(void) { return AppVars.Container.hModel; }

static void ModelContainer_GenerateBBox(
    ModelContainer_t* pContainer, int iFrame, vec3_t& v3Mins, vec3_t& v3Maxs)
{
    v3Mins[0] = -50;
    v3Mins[1] = -50;
    v3Mins[2] = -50;
...
float Model_GetLowestPointOnPrimaryModel(void)
{
    ...
    if (pContainer->hModel) // model loaded?
    {
        ...
        ModelContainer_GenerateBBox(
            pContainer, pContainer->iCurrentFrame_Primary, v3Mins, v3Maxs);
...
static void
ModelDraw_Floor(ModelContainer_t* pContainer, bool bDrawingForOriginalContainer)
{
...
#define FLOOR_TILES_ACROSS 10 // ""
            ...
            glPushAttrib(GL_ENABLE_BIT | GL_CURRENT_BIT | GL_POLYGON_BIT);
            {
                ...
                for (int y = 0; y < FLOOR_TILES_ACROSS; y++)
                {
                    ...
                    {
                        float fX = (FLOOR_TILE_DIM * (float)x) -
                                   (((float)FLOOR_TILES_ACROSS / 2.0f) *
                                    FLOOR_TILE_DIM);
                        float fY = (FLOOR_TILE_DIM * (float)y) -
                                   (((float)FLOOR_TILES_ACROSS / 2.0f) *
                                    FLOOR_TILE_DIM);
                        ...
                        if ((y & 1) ^ (x & 1))
                        {
                            glColor3ub(
                                21,
                                81,
                                32); // a sort of dirty yellow/green check
                        }
                        else
                        {
                        ...
                        glBegin(GL_QUADS);
                        {
                            ...
                            glVertex3f(
                                fX + FLOOR_TILE_DIM, fY + FLOOR_TILE_DIM, fZ);
...
//
static void ModelDraw_BoundingBox(
    ModelContainer_t* pContainer, bool bDrawingForOriginalContainer)
{
    if (AppVars.bBBox && !gbTextInhibit)
    {
        glPushAttrib(GL_ENABLE_BIT | GL_CURRENT_BIT);
        {
            ...
            /*				glBegin(GL_LINE_LOOP);
                            ...
                            glEnd();
            */
            // new version, draw the above shape, but without 3 of the lines
            // (and therefore without GL_LINE_LOOP)
            //	because otherwise the white lines fight with the coloured ones
            //below...  (aesthetics)
            //
            glBegin(GL_LINES);
            {
            ...
            Text_Display(
                va(" X = %.3f", fXDim), v3Pos, 255, 128, 128); // ruler-pink
            //
            // Y...
            //
...
#define iRULER_HEIGHT static_cast<float>((ORIGIN_LINE_RADIUS)*8)
                    ...
                    {
                        glVertex3f(
                            -ORIGIN_LINE_RADIUS, 0.0f, i - (iRULER_HEIGHT / 2));
                ...
                {
                    vec3_t v3 = {
                        -(ORIGIN_LINE_RADIUS + (ORIGIN_LINE_RADIUS / 6)),
                        0,
                        i - (iRULER_HEIGHT / 2)};
                    Text_Display(
                        va("%d __", i - (iRULER_HEIGHT / 2)),
                        v3,
                        255,
                        128,
                        128); // ORIGIN_TEXT_COLOUR);
                }
            }
            glPopAttrib();
        }
...
void PreRenderedMatrixPtrs_glMultiply(void)
{
    ...
    //
    for (int iMatrix = PreRenderedMatrixPtrs.size() - 1; iMatrix >= 0;
         iMatrix--)
    {
...
// specialised use during render process only (assumes current valid GL
// modelview matrix)
//
static bool ModelContainer_ApplyRenderedMatrixToGL(
    ModelContainer_t* pContainer, int iBoltIndex, bool bBoltIsBone)
{
    bool bProceed = false;
    ...
    case MOD_MDXM:
        ...
        if (bBoltIsBone)
        {
            ...
            if (bProceed)
            {
                ...
                GLMatrix_t m;
                BoneMatrix2GLMatrix(
                    &pContainer->XFormedG2Bones[iBoltIndex], &m.matrix[0]);

                // multiply by base pose matrix... (G2 specific)
                //
                GLMatrix_t m2;
                BoneMatrix2GLMatrix(
                    GLMModel_GetBasePoseMatrix(pContainer->hModel, iBoltIndex),
                    &m2.matrix[0]);

                // glMultMatrixf(m);
                // glMultMatrixf(m2);
                PreRenderedMatrixPtrs.push_back(
                    m2); // note reverse order because of push_back (reverse
                         // iteration later)
        ...
        else
        {
            ...
            if (bProceed)
            {
                ...
                GLMatrix_t m;
                BoneMatrix2GLMatrix(
                    &pContainer->XFormedG2TagSurfs[iBoltIndex], &m.matrix[0]);
...
// this goes recursively up the bolting hierarchy and makes a list of matrices
// to multiply against,
//	which we then iterate forwards through to do all the translations...
//(simpler code, but more cpu work)
//
static bool R_ModelContainer_ApplyGLParentMatrices(ModelContainer_t* pContainer)
{
    ...
    if (pContainer->pBoneBolt_ParentContainer)
    {
        bProceed = ModelContainer_ApplyRenderedMatrixToGL(
            pContainer->pBoneBolt_ParentContainer,
            pContainer->iBoneBolt_ParentBoltIndex,
            true); // bBoltIsBone
        if (bProceed)
        {
            bProceed = R_ModelContainer_ApplyGLParentMatrices(
                pContainer->pBoneBolt_ParentContainer);
        }
    }
...
static void ModelContainer_CallBack_AddToDrawList(
    ModelContainer_t* pContainer, void* pvData)
{
    glPushMatrix();
    {
        ...
        if (bProceed)
        {
            ...
            // updateme?
            R_ModView_AddEntity(
                pContainer->hModel,                // ModelHandle_t hModel,
                pContainer->iCurrentFrame_Primary, // int iFrame,
                AppVars.bInterpolate
                    ? pContainer->iOldFrame_Primary
                    : pContainer->iCurrentFrame_Primary, // int iOldFrame,
                pContainer->iBoneNum_SecondaryStart,
                pContainer->iCurrentFrame_Secondary, // int iFrame,
                AppVars.bInterpolate
                    ? pContainer->iOldFrame_Secondary
                    : pContainer->iCurrentFrame_Secondary, // int iOldFrame,
            ...
            gpContainerBeingRendered =
                pContainer; // aaaaarrrgghhh!!! hack city!!!!
            {
            ...
            ModelDraw_BoundingBox(
                pContainer, !!(pContainer == &AppVars.Container));
            ModelDraw_Floor(pContainer, !!(pContainer == &AppVars.Container));
            ModelDraw_OriginLines(!!(pContainer == &AppVars.Container));

            //#####R_ModelContainer_Apply(&AppVars.Container,
            //R_ModelContainer_CallBack_InfoText, &TextData);
...
static void ModelList_AddModelsToDrawList(void)
{
    ...
    //
    R_ModelContainer_Apply(
        &AppVars.Container, ModelContainer_CallBack_AddToDrawList);
...
//
// bHilitAsPure is true if this is a direct highlight, else false when called as
// a referenced bone during surface
//	highlighting. This just affects outputs size and brightness, to make
//things easier to see.

//
static void
DrawTagOrigin(bool bHilitAsPure, const char* psTagText /* can be NULL */)
{
// draw origin lines...
//
#define TAGHIGHLIGHT_LINE_LEN                                                  \
    (bHilitAsPure ? 20.0f : 10.0f) // spot the retro fit... ;-)
#define TAGHIGHLIGHT_TEXT_COLOUR                                               \
    bHilitAsPure ? 255 : 150, 0, bHilitAsPure ? 255 : 150 // ""
#define TAGHIGHLIGHT_TEXT_DISTANCE                                             \
    (bHilitAsPure ? 20.0f : 10.0f) // spot the retro fit... ;-)
    ...
    glPushAttrib(GL_ENABLE_BIT | GL_CURRENT_BIT);
    {
        ...
        if (psTagText)
        {
            ...
            /*static */ vec3_t v3NamePos = {
                TAGHIGHLIGHT_TEXT_DISTANCE,
                TAGHIGHLIGHT_TEXT_DISTANCE,
                TAGHIGHLIGHT_TEXT_DISTANCE};
...
static void
ModelContainer_DrawTagSurfaceHighlights(ModelContainer_t* pContainer)
{
    if (!gbTextInhibit &&
        (AppVars.bSurfaceHighlight &&
         pContainer->iSurfaceHighlightNumber != iITEMHIGHLIGHT_NONE))
    {
        for (int iSurfaceIndex = 0; iSurfaceIndex < pContainer->iNumSurfaces;
             iSurfaceIndex++)
        {
            bool bHighLit =
                (pContainer->iSurfaceHighlightNumber == iSurfaceIndex ||
                 pContainer->iSurfaceHighlightNumber ==
            ...
            if (bHighLit && Model_SurfaceIsTag(pContainer, iSurfaceIndex))
            {
                ...
                //
                const char* psSurfaceName =
                    Model_GetSurfaceName(pContainer->hModel, iSurfaceIndex);

                glPushMatrix();
                {
                    PreRenderedMatrixPtrs.clear();

                    bool bProceed = ModelContainer_ApplyRenderedMatrixToGL(
                        pContainer, iSurfaceIndex, false); // bBoneIsBolt

                    if (bProceed)
                    {
                        // bone wants to be highlighted, and isn't disabled by
                        // virtue of disabled parent surface...
                        //
                        PreRenderedMatrixPtrs_glMultiply();

                        // note special logic for first bool, in other words, if
                        // explicitly highlighting this surface, then draw
                        //	as normal, else if highlighting all tags, then
                        //just do as smaller/dimmer...
                        //
                        DrawTagOrigin(
                            !(pContainer->iSurfaceHighlightNumber ==
                              iITEMHIGHLIGHT_ALL_TAGSURFACES),
...
{
    bool bBoneAliasHighlightingActive =
        (AppVars.bSurfaceHighlight &&
         AppVars.bSurfaceHighlightShowsBoneWeighting &&
         pContainer->iSurfaceHighlightNumber >=
             0); // -ve numbers are NONE, ALL or ALIASED, none of which we want
    ...
    {
        for (int iBoneIndex = 0; iBoneIndex < pContainer->iNumBones;
             iBoneIndex++)
        {
            // this may get called twice, so pre-eval it here for speed...
            //
            const char* psBoneName =
                Model_GetBoneName(pContainer->hModel, iBoneIndex);

            bool bHilitAsPureBone =
                (pContainer->iBoneHighlightNumber == iITEMHIGHLIGHT_ALL ||
                 pContainer->iBoneHighlightNumber == iBoneIndex ||
                 (pContainer->iBoneHighlightNumber == iITEMHIGHLIGHT_ALIASED &&
                  pContainer->Aliases.find(psBoneName) !=
                      pContainer->Aliases.end()));

            bool bHilitAsBoneWeight =
                (bBoneAliasHighlightingActive &&
                 Model_SurfaceContainsBoneReference(
                     pContainer->hModel,
                     AppVars.iLOD,
                     pContainer->iSurfaceHighlightNumber,
                     iBoneIndex));
            ...
            if (bHilitAsPureBone || bHilitAsBoneWeight)
            {
                glPushMatrix();
                {
                    ...
                    bool bProceed = ModelContainer_ApplyRenderedMatrixToGL(
                        pContainer, iBoneIndex, true); // bBoneIsBolt
                    ...
                    if (bProceed)
                    {
                        ...
                        // note special logic for first bool, in other words, if
                        // explicitly highlighting this bone, then draw as
                        // normal,
                        //	else if highlighting as either part of a
                        //boneref, or one of ALL then draw smaller/dimmer...
                        //
                        DrawTagOrigin(
                            (pContainer->iBoneHighlightNumber < 0 &&
                             pContainer->iBoneHighlightNumber !=
                                 iITEMHIGHLIGHT_NONE)
                                ? false
                                : bHilitAsPureBone,
                            va("%s%s%s",
                               ...
                               (!bHilitAsPureBone ||
                                pContainer->iBoneHighlightNumber !=
                                    iITEMHIGHLIGHT_ALIASED)
                                   ? ""
                                   : va("  (Alias: %s)",
                                        pContainer->Aliases[psBoneName]
                                            .c_str()),
...
static Sequence_t* Stats_GetSequenceDisplayInfo(
    ModelContainer_t* pContainer, bool bPrimary, byte* pR = NULL,
    byte* pG = NULL, byte* pB = NULL, bool* pLocked = NULL);
static Sequence_t* Stats_GetSequenceDisplayInfo(
    ModelContainer_t* pContainer, bool bPrimary, byte* pR, byte* pG, byte* pB,
    bool* pLocked)
{
    // return either the locked local frame number in red (if anim locking is
    // on), or just the anim sequence name
    ...
    {
        int iSequenceNumber = Model_MultiSeq_SeqIndexFromFrame(
            pContainer,
            bPrimary ? pContainer->iCurrentFrame_Primary
                     : pContainer->iCurrentFrame_Secondary,
            bPrimary,
            false);
        pLockedSequence = (iSequenceNumber == -1)
                              ? NULL
                              : &pContainer->SequenceList[iSequenceNumber];
    }
    else
    {
        int iSequenceNumber = bPrimary
                                  ? pContainer->iSequenceLockNumber_Primary
                                  : pContainer->iSequenceLockNumber_Secondary;
        pLockedSequence = (iSequenceNumber == -1)
                              ? NULL
                              : &pContainer->SequenceList[iSequenceNumber];
    }
    ...
    {
        // no locked sequence, so print as normal and try and derive the one we
        // just happen to be within...
        //
        if (pLocked)
            *pLocked = false;
        if (pR)
        ...
        pLockedSequence = Sequence_DeriveFromFrame(
            bPrimary ? pContainer->iCurrentFrame_Primary
                     : pContainer->iCurrentFrame_Secondary,
            pContainer);
    }
...
static const char* Stats_GetVertInfo(
    int iVerts, int iTris, int iSurfs, int iXFormedG2Bones,
    int iRenderedBoneWeights, int iOmittedBoneWeights)
{
    return va(
        "(V:%4d T:%4d S:%2d%s)",
        iVerts,
        iTris,
        iSurfs,
        (iXFormedG2Bones != 0) ? va(" XF:%3d WT:%4d%s",
                                    iXFormedG2Bones,
                                    iRenderedBoneWeights,
                                    AppVars.bBoneWeightThreshholdingActive
                                        ? va("+%d", iOmittedBoneWeights)
                                        : "")
                               : "");
}
...
{
    return Stats_GetVertInfo(
        pContainer->iRenderedVerts, // int iVerts,
        pContainer->iRenderedTris,  // int iTris,
        pContainer->iRenderedSurfs, // int iSurfs,
        (pContainer->eModType == MOD_MDXM) ? pContainer->iXformedG2Bones
                                           : 0, // int iXFormedG2Bones
        (pContainer->eModType == MOD_MDXM) ? pContainer->iRenderedBoneWeights
                                           : 0, // int iRenderedBoneWeights,
        (pContainer->eModType == MOD_MDXM) ? pContainer->iOmittedBoneWeights
                                           : 0 // int iOmittedBoneWeights,
    );
}
...
static const char* Stats_GetName(ModelContainer_t* pContainer)
{
    return va(
        "%s",
        Filename_WithoutExt(Filename_WithoutPath(pContainer->sLocalPathName)));
}

static void R_ModelContainer_CallBack_MeasureDigits(
    ModelContainer_t* pContainer, void* pvData)
{
    ...
    if (static_cast<std::size_t>(pTextData->iAttachedViaCharsNeeded) <
        strlen(sTest))
        pTextData->iAttachedViaCharsNeeded = strlen(sTest);

    // "(Secondary Anim)" goes in same place as attachment string, but on line
    // below (if present),
    //	so measure and store against same field...
    //
    if (AppVars.iTotalContainers > 1) // because I don't bother printing the
                                      // header if there's only one model
    {
    ...
    if (AppVars.iTotalContainers > 1 ||
        Model_SecondaryAnimLockingActive(pContainer))
    {
        // measure primary...
        //
        Sequence_t* pSequence = Stats_GetSequenceDisplayInfo(pContainer, true);
        int iLongestSeqString =
            pSequence ? strlen(Sequence_GetName(pSequence, true)) : 0;
    ...
    int iMultiLockSeqs_Secondary = // Model_MultiSeq_IsActive(pContainer,false)?
        ...
        //:0
        ;
    if (Model_SecondaryAnimLockingActive(
            pContainer)) // iMultiLockSeqs_Secondary)
        pTextData->bAnyMultiLockedSecondarySequences = true;

    pTextData->iMostMultiLockedSequences =
        std::max(pTextData->iMostMultiLockedSequences, iMultiLockSeqs_Primary);
    pTextData->iMostMultiLockedSequences = std::max(
        pTextData->iMostMultiLockedSequences, iMultiLockSeqs_Secondary);
}
...
R_ModelContainer_CallBack_InfoText(ModelContainer_t* pContainer, void* pvData)
{
    ...
    //
    {
        // name...
        //
        pTextData->iTextX = Text_DisplayFlat(
            String_EnsureMinLength(
                Stats_GetName(pContainer), pTextData->iModelNameCharsNeeded),
            pTextData->iTextX,
            pTextData->iTextY,
            255,
            255,
        ...
        pTextData->iTextX = Text_DisplayFlat(
            va("Frame: %*d/%*d",
               pTextData->iFrameDigitsNeeded,
               pContainer->iCurrentFrame_Primary,
               pTextData->iFrameDigitsNeeded,
               pContainer->iNumFrames),
            pTextData->iTextX + (2 * TEXT_WIDTH),
            pTextData->iTextY,
            0,
            255,
            0 // RGB (green)
    ...
    //
    {
        int iWhichLOD = (AppVars.iLOD < pContainer->iNumLODs)
                            ? AppVars.iLOD
                            : pContainer->iNumLODs - 1;
        pTextData->iTextX = Text_DisplayFlat(
            String_EnsureMinLength(
                va("(LOD %d/%d)", iWhichLOD + 1, pContainer->iNumLODs),
                ARB_LOD_PADDING),
            pTextData->iTextX + (2 * TEXT_WIDTH),
            pTextData->iTextY,
            255 / 2,
    ...
    //
    {
        pTextData->iTextXForVertStats =
            pTextData->iTextX + (2 * TEXT_WIDTH); // record for "totals" later
        pTextData->iTextX = Text_DisplayFlat(
            String_EnsureMinLength(
                Stats_GetVertInfo(pContainer),
                pTextData->iModelVertInfoCharsNeeded),
            pTextData->iTextXForVertStats,
            pTextData->iTextY,
            255,
            255 / 2,
        ...
        if (pContainer->eModType == MOD_MDXM)
        {
            ...
            pTextData->iTot_RenderedBoneWeights +=
                pContainer->iRenderedBoneWeights;
            pTextData->iTot_OmittedBoneWeights +=
                pContainer->iOmittedBoneWeights;
        }
    }
    ...
    int iAttachedVia_X =
        pTextData->iTextX +
        (2 * TEXT_WIDTH); // also used for "secondary anim" printf later
    {
        pTextData->iTextX = Text_DisplayFlat(
            String_EnsureMinLength(
                Stats_GetAttachmentString(pContainer),
                pTextData->iAttachedViaCharsNeeded),
            iAttachedVia_X,
            pTextData->iTextY,
            0,
            255 / 2,
    ...
    //
    {
        ...
        // primary...
        //
        Sequence_t* pSequence = Stats_GetSequenceDisplayInfo(
            pContainer, true, &_R, &_G, &_B, &bLocked);

        int iPrimarySeqTextX = pTextData->iTextX + (2 * TEXT_WIDTH);
        if (pSequence &&
            !pSequence->bIsDefault) // no point printing default sequence info
        {
            // no space after "Seq: now because Sequence_GetName() prepends with
            // either ' ' or '-'...
            //
            pTextData->iTextX = Text_DisplayFlat(
                va("Seq:%s",
                   String_EnsureMinLength(
                       Sequence_GetName(pSequence, true),
                       pTextData->iSequenceNameCharsNeeded)),
                iPrimarySeqTextX,
                pTextData->iTextY,
                _R,
                _G,
            ...
            pTextData->iTextX = Text_DisplayFlat(
                va("(%d/%d)",
                   pContainer->iCurrentFrame_Primary - pSequence->iStartFrame,
                   pSequence->iFrameCount - 1),
                pTextData->iTextX + (1 * TEXT_WIDTH),
                pTextData->iTextY,
                _R,
                _G,
                _B);
        ...
        if (Model_SecondaryAnimLockingActive(pContainer))
        {
            ...
            if (AppVars.iTotalContainers >
                1) // looks neater if i don't bother printing this when only one
                   // model
            {
                // "(Secondary anim)"...
                //
            ...
            // sequence name...
            //
            pSequence = Stats_GetSequenceDisplayInfo(
                pContainer, false, &_R, &_G, &_B, &bLocked);

            if (pSequence &&
                !pSequence
                     ->bIsDefault) // no point printing default sequence info
            {
                // no space after "Seq: now because Sequence_GetName() prepends
                // with either ' ' or '-'...
                //
                pTextData->iTextX = Text_DisplayFlat(
                    va("Seq:%s",
    ...
    //
    {
        // (AppVars.iTotalContainers>1)	// looks neater if i don't bother
        // printing this when only one model
        for (int iLockPass = 0; iLockPass < 2;
             iLockPass++) // 2 passes, primary and secondary lock
        {
            ...
            {
                // note leading spaces in all text items to match leading-spaces
                // on sequence names (to column-match
                //	for some that have leading '-' on them...
                //
                byte r, g, b;

                //				OutputDebugString(va("most locked =
                //%d\n",pTextData->iMostMultiLockedSequences));
                int iMultiLockedTextY =
                    g_iScreenHeight -
                    ((pTextData->iMostMultiLockedSequences + 1 // <name>
                      + (pTextData->bAnyMultiLockedSecondarySequences
                             ? 1
                             : 0) // <(primary)>
                     ...
                     TEXT_DEPTH);
                //				OutputDebugString(va("iMultiLockedTextY
                //= %d\n",iMultiLockedTextY));
                int iFurthestX = 0;
                ...
                int iTempX = Text_DisplayFlat(
                    va(" %s",
                       Filename_WithoutExt(
                           Filename_WithoutPath(pContainer->sLocalPathName))),
                    pTextData->iMultiLockedTextX,
                    iMultiLockedTextY,
                    r,
                    g,
                    b);
                ...
                if (pTextData->bAnyMultiLockedSecondarySequences &&
                    Model_SecondaryAnimLockingActive(
                        pContainer) // this probably makes the above check
                                    // spurious, but wtf?
                )
                {
                    iTempX = Text_DisplayFlat(
                        !iLockPass ? " ( Primary )" : " ( Secondary )",
                        pTextData->iMultiLockedTextX,
                        iMultiLockedTextY,
                        128,
                        128,
                ...
                //
                int iMultiLockedSequenceWereWithin =
                    Model_MultiSeq_SeqIndexFromFrame(
                        pContainer,
                        !iLockPass ? pContainer->iCurrentFrame_Primary
                                   : pContainer->iCurrentFrame_Secondary,
                        !iLockPass,
                        false);
                for (int iSeqIndex = 0;
                     ...
                     iSeqIndex++)
                {
                    int iSeqEntry = Model_MultiSeq_GetEntry(
                        pContainer, iSeqIndex, !iLockPass);
                    const char* psSeqName =
                        Model_Sequence_GetName(pContainer, iSeqEntry, true);
...
static void ModelDraw_InfoText_Header(void)
{
    ...
    Text_DisplayFlat(
        TextData.sString, 1 * TEXT_WIDTH, 1 * TEXT_DEPTH, 100, 100, 100, false);

    // display main model's skin file data if applicable (SOF2 models only,
    // currently)...
    //
    ...
    sprintf(
        TextData.sString,
        "FPS: %2.2f %s",
        1 / (AppVars.dAnimSpeed),
        AppVars.bAnimate ? "(Playing)" : "(Stopped)");

    int iFPS_Xpos =
        (g_iScreenWidth / 2) - ((strlen(TextData.sString) / 2) * TEXT_WIDTH);

    if (AppVars.bBoneWeightThreshholdingActive)
    {
        const char* psThreshholdString =
            va("BoneWeightThresh: %g%%", AppVars.fBoneWeightThreshholdPercent);
        Text_DisplayFlat(
            psThreshholdString,
            iFPS_Xpos - ((strlen(psThreshholdString) + 2) * TEXT_WIDTH),
            1 * TEXT_DEPTH,
            180,
            180,
            180,
    ...
    TextData.iTextX = Text_DisplayFlat(
        TextData.sString, iFPS_Xpos, 1 * TEXT_DEPTH, 255, 255, 255, false);

    if (AppVars.bInterpolate)
    {
        TextData.iTextX = Text_DisplayFlat(
            "(Interpolated)",
            TextData.iTextX + (2 * TEXT_WIDTH),
            1 * TEXT_DEPTH,
            255 / 2,
            255 / 2,
            255 / 2,
    ...
    // a tacky bit of code that looks nicer on output, basically I want to know
    // how many digits I need for
    ...
    TextData.iMultiLockedTextX =
        1 * TEXT_WIDTH; // was 2 until I added leading spaces to seq-name for
                        // +/- distinction

    R_ModelContainer_Apply(
        &AppVars.Container, R_ModelContainer_CallBack_MeasureDigits, &TextData);
}
...
static void ModelDraw_InfoText_Totals(void)
{
    ...
    if (AppVars.iTotalContainers > 1)
    {
        ...
        strcpy(
            TextData.sString,
            String_EnsureMinLength(
                "( totals )", TextData.iModelNameCharsNeeded));
        TextData.iTextX = Text_DisplayFlat(
            TextData.sString,
            TextData.iTextX,
            TextData.iTextY,
        ...
        TextData.iTextX = Text_DisplayFlat(
            String_EnsureMinLength(
                Stats_GetVertInfo(
                    TextData.iTot_RenderedVerts,  // int iVerts
                    ...
                    TextData
                        .iTot_RenderedBoneWeights,   // int iRenderedBoneWeights
                    TextData.iTot_OmittedBoneWeights // int iOmittedBoneWeights
                    ),
                TextData.iModelVertInfoCharsNeeded),
            TextData.iTextXForVertStats /*TextData.iTextX+(2*TEXT_WIDTH)*/,
            TextData.iTextY,
            255,
            255 / 2,
            255 / 2, // RGB (pink)
    ...
    if (AppVars.bVertWeighting && AppVars.bAtleast1VertWeightDisplayed)
    {
        ...
        for (int i = (AppVars.iSurfaceNumToHighlight == iITEMHIGHLIGHT_ALL) ? 3
                                                                            : 1;
             i < 6;
             i++)
        {
...
// returns appropriate sequence index else -1 for not
// frame-not-found-in-sequences...
//
int Model_MultiSeq_SeqIndexFromFrame(
    ModelContainer_t* pContainer, int iFrame, bool bPrimary, bool bIsOldFrame)
{
    if (bIsOldFrame)
    {
        for (int i = 0; i < Model_MultiSeq_GetNumEntries(pContainer, bPrimary);
             i++)
        {
            ...
            {
                return iSeqIndex; // this sequence is within our multiseq list,
                                  // so ok to return it
            }
        }
    }
...
// similar to above, but returns the entrynum [ 0..<num entries-1> ] that
// container the seq index
int Model_MultiSeq_EntryIndexFromFrame(
    ModelContainer_t* pContainer, int iFrame, bool bPrimary)
{
    ...
    for (int i = 0; i < Model_MultiSeq_GetNumEntries(pContainer, bPrimary); i++)
    {
        ...
        if (iSeqIndex == iThisSeqIndex &&
            Sequence_FrameIsWithin(pSequence, iFrame))
        {
            return i; // this sequence is within our multiseq list, so ok to
                      // return its index within this list
        }
    }
...
// this is really naff, but i needed to add it once I found that workout out the
// sequence number from a frame
//	wasn't working properly because of frames appearing in multiple
//sequences...
//
// Note that this should only be called when Model_MultiSeq_IsActive(), and
// probably also when
    ...
    ModelContainer_t* pContainer, int iFrame, bool bPrimary)
{
    ...
    for (int i = 0; i < Model_MultiSeq_GetNumEntries(pContainer, bPrimary); i++)
    {
        ...
        if (Sequence_FrameIsWithin(pSequence, iFrame))
        {
            ...
            {
                return true; // all is well, our hint is pointing at a sequence
                             // that contains our current frame
            }

            // this isn't our hint, but it's legal for this frame, so preserve
            // in case we need any old legal one later
            //
    ...
    {
        // whoooaaaaa!  Ok, this isn't good, our current frame isn't even within
        // any of the multi-lock sequences,
        //	so presumably the frame legaliser hasn't kicked in yet, so for
        //now, I guess we should just grab the 	first multi-locked sequence?...
        //
        if (Model_MultiSeq_GetNumEntries(pContainer, bPrimary))
        {
            iLegalSeqForThisFrame =
                Model_MultiSeq_GetEntry(pContainer, 0, bPrimary);
        }
        else
        {
...
// gets the next proposed frame in a multilocked sequence, where iStepVal can be
// +ve or -ve, (though usually just -1/0/1),
//	note that this will return a good nextframe number if everything's
//already legal, else it'll just return a non-locked 	number and let the frame
//legaliser sort things out, but be right from then on. Basically think of this
//as a nextframe-num hinter
//
// (this isn't staggeringly fast at certain points, but 99% of the time it
// executes quickly)
//
static int Model_MultiSeq_GetNextFrame(
    ModelContainer_t* pContainer, int iFrame, int iStepVal, bool bPrimary)
{
    if (Model_MultiSeq_IsActive(pContainer, bPrimary))
    {
        if (Model_MultiSeq_GetNumEntries(pContainer, bPrimary))
        {
            if (Model_MultiSeq_EnsureSeqHintLegal(pContainer, iFrame, bPrimary))
            {
                // process the 'StepVal' param in a loop, 1 iteration per
                // loop...
                //
                int iNewFrame = iFrame;
                int iStepDir = (iStepVal < 0) ? -1 : 1;

                for (int iStepValRemaining = iStepVal; iStepValRemaining != 0;
                     iStepValRemaining += -iStepDir)
                {
                    int iThisFrame = iNewFrame;

                    int iMultiLockEntryNum = Model_MultiSeq_EntryIndexFromFrame(
                        pContainer, iThisFrame, bPrimary);
                    if (iMultiLockEntryNum != -1)
                    {
                        Sequence_t* pSeq =
                            &pContainer->SequenceList[Model_MultiSeq_GetEntry(
                                pContainer, iMultiLockEntryNum, bPrimary)];
                        ...
                        if (pSeq)     // itu?
                        {
                            int iNextFrame =
                                iNewFrame +
                                ((pSeq->iFPS < 0) ? -iStepDir : iStepDir);

                            // this is the sequence we're currently in, does the
                            // proposed next frame also sit within this one?...
                            //
                            int iSeqFrameFirst = pSeq->iStartFrame;
                            int iSeqFrameLast =
                                (iSeqFrameFirst + pSeq->iFrameCount) - 1;

                            if (iNextFrame >= iSeqFrameFirst &&
                                iNextFrame <= iSeqFrameLast)
                            {
                                // yes, so adopt it...
                                //
                            ...
                            {
                                // new frame is outside of current group, so get
                                // the index of the sequence after (or before)
                                // this one,
                                //	and adopt the first or last frame of
                                //it...
                                //
                                iMultiLockEntryNum += iStepDir;

                                if (iMultiLockEntryNum >=
                                    Model_MultiSeq_GetNumEntries(
                                        pContainer, bPrimary))
                                ...
                                int iNextSeqIndex = Model_MultiSeq_GetEntry(
                                    pContainer, iMultiLockEntryNum, bPrimary);
                                ...
                                if (pSeq)     // itu?
                                {
                                    Model_MultiSeq_SetSeqHint(
                                        pContainer, bPrimary, iNextSeqIndex);
                                    iSeqFrameFirst = pSeq->iStartFrame;
                                    iSeqFrameLast =
                                        (iSeqFrameFirst + pSeq->iFrameCount) -
                                        1;

                                    if (pSeq->iFPS < 0) // stepping into a
                                                        // backwards sequence?
                                    {
                                        iNewFrame = (iStepDir > 0)
                                                        ? iSeqFrameLast
                                                        : iSeqFrameFirst;
                                    }
                                    else
                                ...
                                {
                                    return iFrame +
                                           iStepVal; // ... let the overall
                                                     // legaliser adjust this
                                                     // frame sometime after
                                                     // returning
                                }
                            }
                        }
                        else
                        {
                            ...
                            //
                            return iFrame +
                                   iStepVal; // ... let the overall legaliser
                                             // adjust this frame sometime after
                                             // returning
                        }
                    }
                    else
                    {
                        // err...
                        return iFrame +
                               iStepVal; // ... let the overall legaliser adjust
                                         // this frame sometime after returning
                    }
                }
            ...
            {
                return iFrame +
                       iStepVal; // ... let the overall legaliser adjust this
                                 // frame sometime after returning
            }
        }
        else
        {
    ...
    else
    {
        ...
        //
        const int iSequenceLockNumber =
            bPrimary ? pContainer->iSequenceLockNumber_Primary
                     : pContainer->iSequenceLockNumber_Secondary;
        const Sequence_t* pSequence =
            (iSequenceLockNumber == -1)
                ? NULL
                : &pContainer->SequenceList[iSequenceLockNumber];
        if (pSequence)
        {
            if (pSequence->iFPS < 0)
            {
...
// does NOT affect original container frame number, just legalises and returns
// it...
//
// new bit, passing in -1 as frame number is used during GoToEndFrame logic for
// legalising to end-of-range, instead of start-of-range
//
int ModelContainer_EnsureFrameLegal(
    ModelContainer_t* pContainer, int iFrame, bool bPrimary, bool bIsOldFrame)
{
    ...
    if (Model_MultiSeq_IsActive(pContainer, bPrimary))
    {
        if (Model_MultiSeq_GetNumEntries(pContainer, bPrimary))
        {
            ...
            //
            if (Model_MultiSeq_SeqIndexFromFrame(
                    pContainer, iFrame, bPrimary, bIsOldFrame) == -1)
            {
                // no, so try and fix it so it is...
                //

                // if we got here then we're outside all current multi-seqs,
                // unfortunately because the specified sequences
                //	can be at any position within the master list order I
                //can't just work out which 2 sequences I'm between 	and jump to
                //the start of the higher one, so working on the principle that
                //99% of the time this code is 	used when lerping between frame &
                //frame+1 I'll see if I can find iFrame-1 anywhere...
                //
                iFrame--;

                // try and find the seq we may have just come from...
                //
                // find the real sequence index, then verify that it's within
                // our multiseq array...
                //
                ...
                {
                    for (int i = 0;
                         i < Model_MultiSeq_GetNumEntries(pContainer, bPrimary);
                         i++)
                    {
                        if (iSeqIndex ==
                            Model_MultiSeq_GetEntry(pContainer, i, bPrimary))
                        {
                            // got it, so is there another sequence after this?
                            //
                            if (++i < Model_MultiSeq_GetNumEntries(
                                          pContainer, bPrimary))
                            {
                                // yes, so adopt first frame of this this seq...
                                //
                                iSeqIndex = Model_MultiSeq_GetEntry(
                                    pContainer, i, bPrimary);
                                if (static_cast<std::size_t>(iSeqIndex) <
                                    pContainer->SequenceList.size())
                                {
                                    if (bLegaliseToStart)
                                        return pContainer
                                            ->SequenceList[iSeqIndex]
                                            .iStartFrame;
                                    return (pContainer->SequenceList[iSeqIndex]
                                                .iStartFrame +
                                            pContainer->SequenceList[iSeqIndex]
                                                .iFrameCount) -
                                           1;
                                }
                                else
                                {
                                    ErrorBox(
                                        va("ModelContainer_EnsureFrameLegal(): "
                                           "Logic error trying to legalise "
                                           "frame %d",
                                           ++iFrame)); // ++ to counter --
                                                       // aboveand therefore get
                                                       // original param
                                }
                            }
                            else
                ...
                // sod it, whatever happened, let's start at the beginning of
                // the list again...
                //
                iSeqIndex = Model_MultiSeq_GetEntry(pContainer, 0, bPrimary);
                Sequence_t* pSeq = &pContainer->SequenceList[iSeqIndex];
                iFrame = bLegaliseToStart
                             ? pSeq->iStartFrame
                             : (pSeq->iStartFrame + pSeq->iFrameCount) - 1;
            }
            else
            {
                // iFrame is within the legal range of one of our multilocked
                // sequences, so don't worry about it...
                //
            }
        }
        else
        ...
        if (pSequence)
        {
            ...
            if (iFrame > (pSequence->iStartFrame + pSequence->iFrameCount) - 1)
            {
                ...
                else
                {
                    ...
                    {
                        iFrame =
                            (pSequence->iStartFrame + pSequence->iFrameCount) -
                            1; // stop at end
                    }
                }
            }
            else if (iFrame < pSequence->iStartFrame)
            {
                ...
                //
                iFrame =
                    (pSequence->iFPS >= 0 && bLegaliseToStart)
                        ? pSequence->iStartFrame
...
static void R_ModelContainer_CallBack_AffectedByLerping(
    ModelContainer_t* pContainer, void* pvData)
{
    bool* pbFeedbackIfTrue = (bool*)pvData;

    if (pContainer->iOldFrame_Primary !=
        pContainer
            ->iCurrentFrame_Primary) // is this model lerping between 2 frames?
    {
...
// called only for speed-opt check for rendering. This just says whether all
// models are unaffected by lerping,
//	eg if every loaded model has the currentframe and oldframe set to the
//same thing...
//
static bool ModelList_AffectedByLerping(void)
{
    ...
    R_ModelContainer_Apply(
        &AppVars.Container,
        R_ModelContainer_CallBack_AffectedByLerping,
        &bReturn);

    return bReturn;
}

static void R_ModelContainer_CallBack_LegaliseFrame(
    ModelContainer_t* pContainer, void* pvData)
{
    ...
    // primary...
    //
    int iFrame = ModelContainer_EnsureFrameLegal(
        pContainer, pContainer->iCurrentFrame_Primary, true, false);
    ...
    iFrame = ModelContainer_EnsureFrameLegal(
        pContainer, pContainer->iOldFrame_Primary, true, true);
    ...
    //
    iFrame = ModelContainer_EnsureFrameLegal(
        pContainer, pContainer->iCurrentFrame_Secondary, false, false);
    ...
    iFrame = ModelContainer_EnsureFrameLegal(
        pContainer, pContainer->iOldFrame_Secondary, false, true);
...
bool ModelList_Animation(void)
{
    if (!gbRenderInhibit)
    {
        ...
        {
            //
            // sanity check, legalise any locked frames in case they were missed
            // elsewhere (dunno why/how that would occur, but...)
            //
            R_ModelContainer_Apply(
                &AppVars.Container,
                R_ModelContainer_CallBack_LegaliseFrame,
                &bModelUpdated);
            ...
            else
            {
                ...
                AppVars.fFramefrac =
                    (float)((dTimeStamp2 - AppVars.dTimeStamp1) / AppVars.dAnimSpeed);
        ...
        // bleurgh...
        //
        bFinalReturn = bNothingUpdated
                           ? gbRedrawNeeded /*false*/
                           : (bFinalReturn || gbRedrawNeeded || bModelUpdated);
...
// sometimes MFC & C++ are a real fucking pain as regards what can talk to what,
// so...

void ModelList_ForceRedraw(void)
{
    gbRedrawNeeded = true; // this was put in so model updated ok when not
                           // animating and treeview toggled a surface
}

// called whenever qdir changes (to avoid using cached textures from other
// dirs), or on program shutdown
...
// UI query function.
//
// Note, since -1 = no locked sequence, you can pass this in to see if NO
// sequences are locked...
//
bool Model_Sequence_IsLocked(
    ModelHandle_t hModel, int iSequenceNumber, bool bPrimary)
{
    ModelContainer_t* pContainer = ModelContainer_FindFromModelHandle(hModel);
...
// UI query function.  ('bUsedForDisplay' signifies that return can be prepended
// with '-' if a reverse sequence)
//
const char* Model_Sequence_GetName(
    ModelHandle_t hModel, int iSequenceNumber,
    bool bUsedForDisplay /* = false */)
{
    ModelContainer_t* pContainer = ModelContainer_FindFromModelHandle(hModel);
    ...
    if (pContainer)
    {
        return Model_Sequence_GetName(
            pContainer, iSequenceNumber, bUsedForDisplay);
    }

    return sERROR_CONTAINER_NOT_FOUND;
}
// ('bUsedForDisplay' signifies that return can be prepended with '-' if a
// reverse sequence)
//
const char* Model_Sequence_GetName(
    ModelContainer_t* pContainer, int iSequenceNumber,
    bool bUsedForDisplay /* = false */)
{
    if (static_cast<std::size_t>(iSequenceNumber) <
        pContainer->SequenceList.size())
    {
        return Sequence_GetName(
            &pContainer->SequenceList[iSequenceNumber], bUsedForDisplay);
    }
    else
    {
        ...
        //
        return va(
            "Model_Sequence_GetName(): Illegal index %d, max = %d!",
            iSequenceNumber,
            pContainer->SequenceList.size() - 1);
    }
}

// returns seq index for supplied name, else -1 for error...
//
int Model_Sequence_IndexForName(
    ModelContainer_t* pContainer, const char* psSeqName)
{
    ...
    ErrorBox(
        va("Model_Sequence_IndexForName(): Unable to resolve index for \"%s\"",
           psSeqName));
    return -1;
}
...
//
const char*
Model_Sequence_GetTreeName(ModelHandle_t hModel, int iSequenceNumber)
{
    ModelContainer_t* pContainer = ModelContainer_FindFromModelHandle(hModel);
    ...
    if (pContainer)
    {
        if (static_cast<std::size_t>(iSequenceNumber) <
            pContainer->SequenceList.size())
        {
            return Sequence_CreateTreeName(
                &pContainer->SequenceList[iSequenceNumber]);
        }
        else
        {
            ...
            //
            return va(
                "Model_Sequence_GetTreeName(): Illegal index %d, max = %d!",
                iSequenceNumber,
                pContainer->SequenceList.size() - 1);
        }
    }
...
// a fairly inefficient function because of what it calls, but it's only used in
// "bursts" rather than every loop,
//	so who cares?...
//
bool Model_Sequence_Lock(
    ModelHandle_t hModel, const char* psSequenceName, bool bPrimary,
    bool bOktoShowErrorBox /* = true */)
{
    ...
    {
        ErrorBox(va(
            "Model_Sequence_Lock(): Unable to find index for sequence \"%s\"!",
            psSequenceName));
    }
    return false;
}
...
bool Model_Sequence_Lock(
    ModelContainer_t* pContainer, int iSequenceNumber, bool bPrimary)
{
    bool bReturn = false;
    ...
    if (pContainer)
    {
            ...
            pContainer->SequenceList.size())
        {
            ...
            {
                if (!Model_SecondaryAnimLockingActive(pContainer))
                    iSequenceNumber = -1; // don't allow secondary locking if no
                                          // scondary bone anim start is set
                pContainer->iSequenceLockNumber_Secondary = iSequenceNumber;
            }

            ModelList_StepFrame(
                0, false); // to legalise old frame so if no anim and you lock
                           // to another sequence then you don't try and lerp to
                           // an old one outside that sequence
            bReturn = true;
        }
        else
        {
            ErrorBox(
                va("Attempting to lock sequence # %d when max is %d!",
                   iSequenceNumber,
                   pContainer->SequenceList.size() - 1));
        }
    }
...
static void
R_ModelContainer_CallBack_Rewind(ModelContainer_t* pContainer, void* pvData)
{
    pContainer->iCurrentFrame_Primary =
        ModelContainer_EnsureFrameLegal(pContainer, 0, true, false);
    pContainer->iCurrentFrame_Secondary =
        ModelContainer_EnsureFrameLegal(pContainer, 0, false, false);
}
...
{
    R_ModelContainer_Apply(
        &AppVars.Container, R_ModelContainer_CallBack_Rewind);

    ModelList_ForceRedraw();
}

static void R_ModelContainer_CallBack_GoToEndFrame(
    ModelContainer_t* pContainer, void* pvData)
{
    pContainer->iCurrentFrame_Primary =
        ModelContainer_EnsureFrameLegal(pContainer, -1, true, false);
    pContainer->iCurrentFrame_Secondary =
        ModelContainer_EnsureFrameLegal(pContainer, -1, false, false);
}

void ModelList_GoToEndFrame()
{
    R_ModelContainer_Apply(
        &AppVars.Container, R_ModelContainer_CallBack_GoToEndFrame);
...
static void R_ModelContainer_CallBack_UpdateFrame(
    ModelContainer_t* pContainer, void* pvData)
{
    ...
    //
    {
        ...
        //
        pContainer->iOldFrame_Primary = ModelContainer_EnsureFrameLegal(
            pContainer,
            pContainer->iCurrentFrame_Primary,
            true,
            true); // for locking
        pContainer->iCurrentFrame_Primary = Model_MultiSeq_GetNextFrame(
            pContainer,
            pContainer->iCurrentFrame_Primary,
            pStepFrameData->iStepVal,
            true);
        pContainer->iCurrentFrame_Primary = ModelContainer_EnsureFrameLegal(
            pContainer, pContainer->iCurrentFrame_Primary, true, false);
    ...
    if (Model_SecondaryAnimLockingActive(pContainer))
    {
        ...
        // this is the only place in the code where this gets updated...
        //
        pContainer->iOldFrame_Secondary = ModelContainer_EnsureFrameLegal(
            pContainer,
            pContainer->iCurrentFrame_Secondary,
            false,
            true); // for locking
        pContainer->iCurrentFrame_Secondary = Model_MultiSeq_GetNextFrame(
            pContainer,
            pContainer->iCurrentFrame_Secondary,
            pStepFrameData->iStepVal,
            false);
        pContainer->iCurrentFrame_Secondary = ModelContainer_EnsureFrameLegal(
            pContainer, pContainer->iCurrentFrame_Secondary, false, false);
...
bool ModelList_StepFrame(int iStepVal, bool bAutoAnimOff /* = true */)
{
    ...
    StepFrameData.bSomethingHasUpdated =
        false; // used for draw-optimising so when anim is on but all models are
               // at non-wrap endframe, then no screen update

    R_ModelContainer_Apply(
        &AppVars.Container,
        R_ModelContainer_CallBack_UpdateFrame,
        &StepFrameData);
...
// note that it's only valid to call this straight after you've called
// Model_GetNumAliases(), if you add
//	any more to this then the indexes are invalidated. ie, only call during
//an interrogate loop, and don't 	store the indexes anywhere!
//
bool Model_GetBoneAliasPair(
    ModelHandle_t hModel, int iAliasIndex, std::string& strRealName,
    std::string& strAliasName)
{
    ...
    if (pContainer)
    {
        return ModelContainer_GetBoneAliasPair(
            pContainer, iAliasIndex, strRealName, strAliasName);
    }
...
//an interrogate loop, and don't 	store the indexes anywhere!
//
bool ModelContainer_GetBoneAliasPair(
    ModelContainer_t* pContainer, int iAliasIndex, std::string& strRealName,
    std::string& strAliasName)
{
    ...
    assert(0);
    ErrorBox(
        va("Illegal alias index %d, max = %d",
           iAliasIndex,
           pContainer->Aliases.size()));
    return false;
}
...
// remote helper func only...    returns a string with sequence name,
// startframe, duration etc
//
// NULL return = error
//
const char* Model_GetSequenceString(ModelHandle_t hModel, int iSequenceNum)
{
    ...
    if (pContainer && static_cast<std::size_t>(iSequenceNum) <
                          pContainer->SequenceList.size())
    {
        return Sequence_ReturnRemoteQueryString(
            &pContainer->SequenceList[iSequenceNum]);
    }
...
// I don't bother with validating the index because if it's wrong then it just
...
bool Model_SetSurfaceHighlight(ModelHandle_t hModel, int iSurfaceIndex)
{
    ...
    {
        pContainer->iSurfaceHighlightNumber = iSurfaceIndex;
        AppVars.iSurfaceNumToHighlight =
            iSurfaceIndex; // a bit tacky, but saves passing container info down
                           // the callstack
...
bool Model_SetSurfaceHighlight(ModelHandle_t hModel, const char* psSurfaceName)
{
    ...
    if (pContainer)
    {
        // find this surface (fortunately, all names are unique within a
        // model)...
        //
        for (int iSurface = 0; iSurface < pContainer->iNumSurfaces; iSurface++)
        {
            if (!Q_stricmp(
                    psSurfaceName,
                    pContainer->pModelGetSurfaceNameFunction(hModel, iSurface)))
            {
...
//
// this is now slightly recursive, at least if you supply an aliased bone
// name...
//
int ModelContainer_BoneIndexFromName(
    ModelContainer_t* pContainer, const char* psBoneName)
{
    int iBone = 0;
    ...
    for (iBone = 0; iBone < pContainer->iNumBones; iBone++)
    {
        if (!Q_stricmp(
                psBoneName,
                pContainer->pModelGetBoneNameFunction(
                    pContainer->hModel, iBone)))
        {
            return iBone;
        }
    }
    ...
    if (!bAlreadyHere)
    {
        // new bit, if we didn't find the bone name, check the aliases (I could
        // interrogate the tree, but this
        //	seems nicer...
        //
        // (Note that the alias <map> is opposite to what you'd think, ie
        // [realname] = aliasname.
        //
        // This makes for slower name lookup on these occasional calls, but
        // faster is-alias-present-for-this-bone
        //	checks during rendering.
        //
        /*
        for (MappedString_t::iterator it = pContainer->Aliases.begin(); it !=
        pContainer->Aliases.end(); ++it)
        {
                CString strBoneName_Real (((*it).first).c_str());
                CString strBoneName_Alias(((*it).second).c_str());

                if (!stricmp(((*it).second).c_str(), psBoneName))	// bone
        name arg matches this alias?
                {
                        bAlreadyHere = true;
                        iBone = ModelContainer_BoneIndexFromName(pContainer,
        ((*it).first).c_str());	// pass real bone name bAlreadyHere = false;
        ...
        */
        ...
        for (int i = 0; i < iAliases; i++)
        {
            ...
            if (ModelContainer_GetBoneAliasPair(
                    pContainer, i, strBoneNameReal, strBoneNameAlias))
            {
                if (!Q_stricmp(strBoneNameAlias.c_str(), psBoneName))
                {
                    bAlreadyHere = true;
                    iBone = ModelContainer_BoneIndexFromName(
                        pContainer,
                        strBoneNameReal.c_str()); // pass real bone name
                    bAlreadyHere = false;
...
bool Model_SetBoneHighlight(ModelHandle_t hModel, const char* psBoneName)
{
    ...
    {
        int iBoneIndex =
            ModelContainer_BoneIndexFromName(pContainer, psBoneName);
        if (iBoneIndex != -1)
        {
...
void Model_StopAnim() { AppVars.bAnimate = false; }

// for use with script files and remote control, since it's safer to use bone
// names rather than indexes, in case they change
...
bool Model_SetSecondaryAnimStart(ModelHandle_t hModel, int iBoneNum)
{
    ...
    if (pContainer)
    {
        if (iBoneNum == 0) // UI-update check on popup menu will not allow 0 to
                           // be sent, but remote-commands etc can do it.
        {
            WarningBox("Model_SetSecondaryAnimStart(): Attempting to start "
                       "seconday anim from bone 0\n\nThis is probably bad, and "
                       "is almost certainly pointless anyway");
            iBoneNum = -1; // clear it
        }
        else if (iBoneNum >= pContainer->iNumBones)
        {
            ErrorBox(
                va("Model_SetSecondaryAnimStart(): attempting to set start on "
                   "bone %d, but max = %d!",
                   iBoneNum,
                   pContainer->iNumBones - 1));
            iBoneNum = -1; // clear it
        }
        else
        {
...
bool Model_MultiSeq_AlreadyContains(
    ModelContainer_t* pContainer, int iSequenceNumber, bool bPrimary)
{
    for (int i = 0; i < Model_MultiSeq_GetNumEntries(pContainer, bPrimary); i++)
    {
    ...
    ModelHandle_t hModel, int iSequenceNumber, bool bPrimary)
{
    ...
    if (pContainer)
    {
        return Model_MultiSeq_AlreadyContains(
            pContainer, iSequenceNumber, bPrimary);
    }
    else
    {
        assert(0);
    ...
    bool bActivate /* = true */)
{
    ...
    if (pContainer)
    {
        if (static_cast<std::size_t>(iSequenceNumber) <
            pContainer->SequenceList.size())
        {
            if (!Model_MultiSeq_AlreadyContains(
                    pContainer, iSequenceNumber, bPrimary))
            {
                if (bPrimary)
                {
                ...
                {
                    pContainer->SeqMultiLock_Secondary.push_back(
                        iSequenceNumber);
                    if (bActivate)
                    {
        ...
        {
            ErrorBox(
                va("Model_MultiSeqLock_Add(): Illegal sequence number %d",
                   iSequenceNumber));
        }
    }
    else
...
bool Model_SetG2SurfaceRootOverride(
    ModelContainer_t* pContainer, const char* psSurfaceName)
{
    for (int iSurface = 0; iSurface < pContainer->iNumSurfaces; iSurface++)
    {
        const char* psScannedSurfaceName =
            Model_GetSurfaceName(pContainer, iSurface);
...
//
bool Model_SetG2SurfaceRootOverride(
    ModelContainer_t* pContainer, int iSurfaceNum)
{
    bool bReturn = false;
    ...
    {
        ErrorBox(
            va("Model_SetG2SurfaceRootOverride(): attempting to set surface %d "
               "as root, but max = %d!",
               iSurfaceNum,
               pContainer->iNumSurfaces - 1));
    }
    else
    {
...
int Model_MultiSeq_GetEntry(
    ModelContainer_t* pContainer, int iEntry, bool bPrimary)
{
    if (bPrimary)
    {
        if (static_cast<std::size_t>(iEntry) <
            pContainer->SeqMultiLock_Primary.size())
            return pContainer->SeqMultiLock_Primary[iEntry];
    }
    else
    {
        if (static_cast<std::size_t>(iEntry) <
            pContainer->SeqMultiLock_Secondary.size())
            return pContainer->SeqMultiLock_Secondary[iEntry];
    }

    ErrorBox(
        va("Model_MultiSeq_GetEntry() illegal %s index %d ( max is %d )",
           bPrimary ? "primary" : "secondary",
           iEntry,
           bPrimary ? pContainer->SeqMultiLock_Primary.size() - 1
                    : pContainer->SeqMultiLock_Secondary.size() - 1));
    return NULL;
}
...
// this is a tacky way of getting round the fact of not having a proper GUI for
// editing these lists,
//	basically you just delete the last thing entered if you make a
//mistake...
//
void Model_MultiSeq_DeleteLast(ModelHandle_t hModel, bool bPrimary)
{
    ...
    if (pContainer)
    {
        ...
        ModelList_ForceRedraw(); // important, in case current frame was within
                                 // locked-seq we just deleted from set
        return;
    }
...
void Model_MultiSeq_Delete(ModelHandle_t hModel, int iSeqIndex, bool bPrimary)
{
    ...
    if (pContainer)
    {
        // now we have to turn this sequence index into a multilock list
        // index...
        //
        for (int i = 0; i < Model_MultiSeq_GetNumEntries(pContainer, bPrimary);
             i++)
        {
            ...
            if (iThisSeqIndex == iSeqIndex)
            {
                if (bPrimary)
                {
                    ...
                    {
                        pContainer->SeqMultiLock_Primary.erase(
                            pContainer->SeqMultiLock_Primary.begin() + i);
                    }
                }
                else
                {
                    if (pContainer->SeqMultiLock_Secondary.size() >
                        static_cast<std::size_t>(i))
                    {
                        pContainer->SeqMultiLock_Secondary.erase(
                            pContainer->SeqMultiLock_Secondary.begin() + i);
                    }
                }
            }
        }
...
void Model_MultiSeq_SetActive(ModelHandle_t hModel, bool bPrimary, bool bActive)
{
    ...
    if (pContainer)
    {
        ...
        ModelList_ForceRedraw(); // even just unlocking can affect lerping, so
                                 // redraw needed
        return;
    }
...
{
    return GLMModel_EnsureGenerated_VertEdgeInfo(
        pContainer->hModel, iLOD, pContainer->SurfaceEdgeInfoPerLOD);
}

// return value is the corrected (if over the model limit) LOD level, -1 =
// failure !
//
...
static void BuildTangentVectors(
    mdxmVertex_t* v, int numVerts, mdxmTriangle_t* tri, int numTriangles)
{
    ...
    for (i = 0; i < numTriangles; i++)
    {
        ...
        vec1[0] = v[tri[i].indexes[1]].vertCoords[0] -
                  v[tri[i].indexes[0]].vertCoords[0];
        vec1[1] = pTex[tri[i].indexes[1]].texCoords[0] -
                  pTex[tri[i].indexes[0]].texCoords[0];
        vec1[2] = pTex[tri[i].indexes[1]].texCoords[1] -
                  pTex[tri[i].indexes[0]].texCoords[1];

        vec2[0] = v[tri[i].indexes[2]].vertCoords[0] -
                  v[tri[i].indexes[0]].vertCoords[0];
        vec2[1] = pTex[tri[i].indexes[2]].texCoords[0] -
                  pTex[tri[i].indexes[0]].texCoords[0];
        vec2[2] = pTex[tri[i].indexes[2]].texCoords[1] -
                  pTex[tri[i].indexes[0]].texCoords[1];
        ...
        vec1[0] = v[tri[i].indexes[1]].vertCoords[1] -
                  v[tri[i].indexes[0]].vertCoords[1];
        vec1[1] = pTex[tri[i].indexes[1]].texCoords[0] -
                  pTex[tri[i].indexes[0]].texCoords[0];
        vec1[2] = pTex[tri[i].indexes[1]].texCoords[1] -
                  pTex[tri[i].indexes[0]].texCoords[1];

        vec2[0] = v[tri[i].indexes[2]].vertCoords[1] -
                  v[tri[i].indexes[0]].vertCoords[1];
        vec2[1] = pTex[tri[i].indexes[2]].texCoords[0] -
                  pTex[tri[i].indexes[0]].texCoords[0];
        vec2[2] = pTex[tri[i].indexes[2]].texCoords[1] -
        ...
        vec1[0] = v[tri[i].indexes[1]].vertCoords[2] -
                  v[tri[i].indexes[0]].vertCoords[2];
        vec1[1] = pTex[tri[i].indexes[1]].texCoords[0] -
                  pTex[tri[i].indexes[0]].texCoords[0];
        vec1[2] = pTex[tri[i].indexes[1]].texCoords[1] -
                  pTex[tri[i].indexes[0]].texCoords[1];

        vec2[0] = v[tri[i].indexes[2]].vertCoords[2] -
                  v[tri[i].indexes[0]].vertCoords[2];
        vec2[1] = pTex[tri[i].indexes[2]].texCoords[0] -
                  pTex[tri[i].indexes[0]].texCoords[0];
        vec2[2] = pTex[tri[i].indexes[2]].texCoords[1] -
...
bool Model_Save(const char* psFullPathedFilename)
{
    ...
    mdxmHeader_t* pHeader =
        (mdxmHeader_t*)RE_GetModelData(AppVars.hModelLastLoaded);
    if (!pHeader)
        return false;
    ...
    for (l = 0; l < pHeader->numLODs; l++)
    {
        ...
        mdxmLODSurfOffset_t* indexes =
            (mdxmLODSurfOffset_t*)((char*)lod + sizeof(mdxmLOD_t));
        newindexes = new mdxmLODSurfOffset_t[pHeader->numSurfaces];
        memcpy(
            newindexes,
            indexes,
            sizeof(mdxmLODSurfOffset_t) * pHeader->numSurfaces);

        // iterate thru the surfaces
        surf =
            (mdxmSurface_t*)((byte*)lod + sizeof(mdxmLOD_t) + (pHeader->numSurfaces * sizeof(mdxmLODSurfOffset_t)));

        for (i = 0; i < pHeader->numSurfaces; i++)
        {
            // Calculate the memory savings for this surface
            int save = (surf->numVerts *
                        (sizeof(mdxmVertex_t) + sizeof(mdxmVertexTexCoord_t))) -
                       (surf->numVerts * (sizeof(mdxmVertexComp_t) +
                                          sizeof(mdxmVertexTexCoordComp_t)));
        ...
        for (i = 0; i < pHeader->numSurfaces; i++)
        {
            ...
            for (int ver = 0; ver < surf->numVerts; ver++)
            {
                ...
                compv.vertCoords[0] =
                    (short)(LittleFloat(v->vertCoords[0]) * POINT_SCALE);
                compv.vertCoords[1] =
                    (short)(LittleFloat(v->vertCoords[1]) * POINT_SCALE);
                compv.vertCoords[2] =
                    (short)(LittleFloat(v->vertCoords[2]) * POINT_SCALE);

                /*compv.normal[0] = (short)(LittleFloat( v->normal[0] ) *
                POINT_SCALE ); compv.normal[1] = (short)(LittleFloat(
                v->normal[1] ) * POINT_SCALE ); compv.normal[2] =
                (short)(LittleFloat( v->normal[2] ) * POINT_SCALE );*/

                /*compv.normal = ( ( ((unsigned int)(v->normal[2] *  511.0f)) &
                0x3ff ) << 22L ) | ( ( ((unsigned int)(v->normal[1] * 1023.0f))
                & 0x7ff ) << 11L ) | ( ( ((unsigned int)(v->normal[0] *
                1023.0f)) & 0x7ff ) <<  0L );

                compv.tangent = ( ( ((unsigned int)(_tangents[ver][2] * 511.0f))
                & 0x3ff ) << 22L ) | ( ( ((unsigned int)(_tangents[ver][1] *
                1023.0f)) & 0x7ff ) << 11L ) | ( ( ((unsigned
                int)(_tangents[ver][0] * 1023.0f)) & 0x7ff ) <<  0L );*/

                unsigned int ix =
                    (unsigned int)((v->normal[0] * 127.f) + 128.f);
                unsigned int iy =
                    (unsigned int)((v->normal[1] * 127.f) + 128.f);
                unsigned int iz =
                    (unsigned int)((v->normal[2] * 127.f) + 128.f);
                unsigned int iw =
                    0; // we don't use the w component (just padding)
            ...
            mdxmVertexTexCoord_t* pTexCoords =
                (mdxmVertexTexCoord_t*)&v[surf->numVerts];
            //			fwrite(pTexCoords, 1, sizeof(mdxmVertexTexCoord_t) *
            //surf->numVerts, pFile);
