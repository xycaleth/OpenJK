...
void LoadJPG(const char* filename, unsigned char** pic, int* width, int* height)
{
    *pic = NULL;
    ...
    // this JPG loader is a bit crap, and often causes read-violations because
    // of reading an extra 4k from the input
    //	buffer that isn't actually there, causes problems on NT4 and NT2000, so
    //allocate 4K more and use that instead.
    //
    ...
    try
    {
         ...
         * memory.) This routine fills in the contents of struct jerr, and
         * returns jerr's address which we place into the link field in cinfo.
         */
        ...
        *height = cinfo.output_height;
        ...
        if (cinfo.output_components == 1)
        {
            ...
            byte* pbSrc =
                (*pic + (cinfo.output_width * cinfo.output_height)) - 1;
