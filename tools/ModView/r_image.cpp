...
typedef struct
{
        ...
        byte	byColourmapType;	// 0 = truecolour, 1 = paletted, else
bad
        byte	byImageType;		// 1 = colour mapped (palette),
uncompressed, 2 = truecolour, uncompressed, else bad
        word	w1stColourMapEntry;	// must be 0
        word	wColourMapLength;	// 256 for 8-bit palettes, else 0 for
true-colour
        byte	byColourMapEntrySize; // 24 for 8-bit palettes, else 0 for
true-colour word	wImageXOrigin;		// ignored word wImageYOrigin;
// ignored word	wImageWidth;		// in pixels word	wImageHeight;
// in pixels
        byte	byImagePlanes;		// bits per pixel	(8 for paletted,
else 24 for true-colour) byte	byScanLineOrder;	// Image descriptor
bytes
                                                                // bits 0-3 = #
attr bits (alpha chan)
                                                                // bits 4-5 =
pixel order/dir
                                                                // bits 6-7 scan
line interleave (00b=none,01b=2way interleave,10b=4way) } TGAHEADER,
*LPTGAHEADER; #pragma pack(pop)

*/
bool TGA_Write(
    const char* psFullPathedFilename, byte* pPixels, int iWidth, int iHeight,
    int iPlanes)
{
    byte* pbBuffer = NULL;
    ...
    pbBuffer[17] = 0x00; // scan line order, tells TGA-legal packages that it's
                         // upside down. ID code ignore it.

    memcpy(pbBuffer + 18, pPixels, iTotalPixelBytes);

    // turn it upside down to match TGA default, because ID loader ignores
    // scanline attr flags... (sigh)
    //
...
void LoadTGA ( const char *name, byte **pic, int *width, int *height)
{
        ...
        *pic = NULL;
        ...
        {
                ri.Error (ERR_DROP, "LoadTGA: Only type 2 (RGB), 3 (gray), and
10 (RGB) TGA images supported\n");
        }
...
&& targa_header.image_type != 3 )
        {
                ri.Error (ERR_DROP, "LoadTGA: Only 32 or 24 bit images supported
(no colormaps)\n");
        }
                        ...
                        for(column=0; column<columns; column++)
                        {
                                ...
                                case 32:
                                        ...
                                        *pixbuf++ = alphabyte;
                                        break;
                                default:
                                        ri.Error( ERR_DROP, "LoadTGA: illegal
pixel_size '%d' in file '%s'\n", targa_header.pixel_size, name ); break;
                                }
                        }
                }
        }
        else if (targa_header.image_type==10) {   // Runlength encoded RGB
images unsigned char red,green,blue,alphabyte,packetHeader,packetSize,j;
                ...
                for(row=rows-1; row>=0; row--) {
                        ...
                        for(column=0; column<columns; ) {
                                ...
                                if (packetHeader & 0x80) {        // run-length
packet switch (targa_header.pixel_size) { case 24: blue = *buf_p++; green =
*buf_p++; red = *buf_p++; alphabyte = 255; break; case 32: blue = *buf_p++;
                                                                green =
*buf_p++; red = *buf_p++; alphabyte = *buf_p++; break; default: ri.Error(
ERR_DROP, "LoadTGA: illegal pixel_size '%d' in file '%s'\n",
targa_header.pixel_size, name ); break;
                                        }
                                        ...
                                        for(j=0;j<packetSize;j++) {
                                                ...
                                                *pixbuf++=alphabyte;
                                                column++;
                                                if (column==columns) { // run
spans across rows column=0; if (row>0) row--; else goto breakOut; pixbuf = pRGBA
+ row*columns*4;
                                                }
                                        }
                                }
                                else {                            // non
run-length packet for(j=0;j<packetSize;j++) { switch (targa_header.pixel_size) {
                                                        case 24:
                                                                        blue =
*buf_p++; green = *buf_p++; red = *buf_p++; *pixbuf++ = red; *pixbuf++ = green;
                                                                        *pixbuf++
= blue; *pixbuf++ = 255; break; case 32: blue = *buf_p++; green = *buf_p++; red
= *buf_p++; alphabyte = *buf_p++; *pixbuf++ = red; *pixbuf++ = green; *pixbuf++
= blue; *pixbuf++ = alphabyte; break; default: ri.Error( ERR_DROP, "LoadTGA:
illegal pixel_size '%d' in file '%s'\n", targa_header.pixel_size, name ); break;
                                                }
                                                column++;
                                                if (column==columns) { // pixel
packet run spans across rows column=0; if (row>0) row--; else goto breakOut;
                                                        pixbuf = pRGBA +
row*columns*4;
                                                }
                                        }
...
#define TGA_FORMAT_ERROR(blah)                                                 \
    ...
    {
        TGA_FORMAT_ERROR("LoadTGA: Only type 2 (RGB), 3 (gray), and 10 "
                         "(RLE-RGB) images supported\n");
    }
    ...
    {
        TGA_FORMAT_ERROR(
            "LoadTGA: ColourMapEntrySize must be either 0 or 24\n");
    }

    if ((pHeader->byImagePlanes != 24 && pHeader->byImagePlanes != 32) &&
        (pHeader->byImagePlanes != 8 && pHeader->byImageType != 3))
    {
        TGA_FORMAT_ERROR("LoadTGA: Only type 2 (RGB), 3 (gray), and 10 (RGB) "
    ...
    // these last checks are so i can use ID's RLE-code. I don't dare fiddle
    // with it or it'll probably break...
    //
    ...
    *pic = pRGBA;
    ...
    // I don't know if this ID-thing here is right, since comments that I've
    // seen are at the end of the file,
        ...
        pHeader->byImageType == 3) // RGB or greyscale
    {
             ...
             y += iYStep, iYCount++)
        {
                 ...
                 x += iXStep, iXCount++)
            {
                ...
                default:
                    assert(0); // if we ever hit this, someone deleted a header
                               // check higher up
                    TGA_FORMAT_ERROR("LoadTGA: Image can only have 8, 24 or 32 "
                                     "planes for RGB/greyscale\n");
                    break;
                }
            }
        }
    ...
    {
        // I've no idea if this stuff works, I normally reject RLE targas, but
        // this is from ID's code
        ...
        for (int y = pHeader->wImageHeight - 1; y >= 0; y--)
        {
            ...
            for (int x = 0; x < pHeader->wImageWidth;)
            {
                ...
                if (packetHeader & 0x80) // run-length packet
                {
                    ...
                    default:
                        assert(0); // if we ever hit this, someone deleted a
                                   // header check higher up
                        TGA_FORMAT_ERROR(
                ...
                { // non run-length packet
                    ...
                    for (j = 0; j < packetSize; j++)
                    {
                        ...
                        default:
                            assert(0); // if we ever hit this, someone deleted a
                                       // header check higher up
                            TGA_FORMAT_ERROR("LoadTGA: RLE-RGB can only have "
                                             "24 or 32 planes\n");
                            break;
                        }
                        x++;
                        if (x == pHeader->wImageWidth) // pixel packet run spans
                                                       // across rows
                        {
                            x = 0;
                            if (y > 0)
...
*/
static void LoadPCX(
    const char* filename, byte** pic, byte** palette, int* width, int* height)
{
    byte* raw;
    pcx_t* pcx;
    int x, y;
...
// feeder version of above because of John's not having extensions on anymore
// (but it might do... sigh)
//
...
void R_LoadImage1(const char* name, byte** pic, int* width, int* height)
{
    ...
    *height = 0;
    ...
    if (name[1] == ':')
    {
        ...
        {
            g_bReportImageLoadErrors = GetYesNo(
                va("Filename \"%s\" has a full-path in it, paths should be "
                   "local only!\n\nContinue reporting image-load errors?",
                   name));
        }
        return;
    }
    ...
    // else if not found, or no extension specified, then force some on in SOF2
    // order...
    //
    ...
    {
        g_bReportImageLoadErrors = GetYesNo(
            va("R_LoadImage(): Couldn't read image:\n\n\"%s\"\n\n... continue "
               "receiving this message for this model?",
               name));
    }
}
...
void R_LoadImage(const char* name, byte** pic, int* width, int* height)
{
    ...
    {
        // special case, so just zap whatever the last extension found was, else
        // it gets strcat'd on return...
        //
