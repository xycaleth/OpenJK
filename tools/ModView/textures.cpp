...
bool GetFileTime(const char* psFileName, time_t& ft)
{
    ...
    if (psFileName[1] != ':')
    {
        ...
        if (!gamedir[0])
        {
            // Com_Error( ERR_FATAL, "Filesystem call made without
            // initialization\n" );
            return false;
        }
...
// cvars
cvar_t *r_simpleMipMaps, *r_roundImagesDown, *r_picmipmin, *r_picmip,
    *r_texturebits, *r_ext_compressed_lightmaps, *r_colorMipLevels;
...
// TODO:  need API for texture deletion, particularly upon qdir change (so we
// don't end up with textures from other
...
static TextureHandle_t TextureList_GetNewHandle(void) { return thNextHandle++; }
...
TextureHandle_t TextureHandle_ForName(const char* psLocalTexturePath)
{
    std::string strTextureName =
        String_ToLower(Filename_WithoutExt(psLocalTexturePath));
...
{
    for (TextureList_t::iterator it = TheTextures.begin();
         it != TheTextures.end();
         ++it)
    {
...
void Texture_SetFilter(void)
{
    if (AppVars.bBilinear)
    {
        ...
        glTexParameterf(
            ...
            GL_LINEAR); // hitech: GL_LINEAR_MIPMAP_NEAREST
...
static void ResampleTexture(
    unsigned* in, int inwidth, int inheight, unsigned* out, int outwidth,
    int outheight)
{
    int i, j;
...
static void Upload32(
    unsigned* data, int width, int height, qboolean mipmap, qboolean picmip,
    qboolean isLightmap, int* format, int* pUploadWidth, int* pUploadHeight)
{
    int samples;
    ...
    if (scaled_width != width || scaled_height != height)
    {
        ...
        ResampleTexture(
            data, width, height, resampledBuffer, scaled_width, scaled_height);
    ...
    if (picmip)
    {
        if (r_picmipmin->integer < scaled_width &&
            r_picmipmin->integer < scaled_height)
        { // only do it if it would be smaller
            //			scaled_width >>= r_picmip->integer;
            //			scaled_height >>= r_picmip->integer;
            int count = r_picmip->integer;
            // mip down and clamp to minimum picmipsize
            while (count && (scaled_width > r_picmipmin->integer ||
    ...
    if (samples == 3)
    {
        ...
        internalFormat = 3; // GL allows you to specify the number of color
                            // components in the texture like this

        if ((glConfig.textureCompression != TC_NONE && !isLightmap) ||
            (glConfig.textureCompression != TC_NONE && isLightmap &&
             r_ext_compressed_lightmaps->integer))
        {
            /*modviewrem			if ( glConfig.textureCompression == TC_S3TC
                                    ...
                                    else if ( glConfig.textureCompression ==
               TC_S3TC_DXT )
                                    {
    ...
    {
        // Set the default format, then override as necessary
        internalFormat = 4; // GL allows you to specify the number of color
                            // components in the texture like this
    ...
    // modviewrem R_LightScaleTexture (scaledBuffer, scaled_width,
    // scaled_height, !mipmap );
...
// subroutinised so I can add picmip stuff, called from both texture loader and
// textures-refresh...
//
static void Texture_BindAndUpload(Texture_t* pTexture)
{
    ...
    /*
       ...
       level,		(LOD)
                                            GL_RGBA8,			// GLint
       components / internal dest format pTexture->iWidth,	// GLsizei
       width, pTexture->iHeight,	// GLsizei height, 0,
       // GLint border, GL_RGBA,			// GLenum format (src)
                                            GL_UNSIGNED_BYTE,	// GLenum type,
                                            pTexture->pPixels	// const GLvoid
       *pixels
                                            );
    */
    // newcode
    ...
    // since mipping overwrites original buffer data, I'm going to have to do
    // all ops on a scratchpad in this app...
    //
    ...
    Upload32(
        (unsigned int*)gpiReMipScratchBuffer, // pTexture->pPixels,	//
                                              // unsigned *data
...
int TextureList_GetMip(void) { return r_picmip->integer; }
                ...
                /*
                   ...
                   false;
                                                        //if (!bDone)
                                                        {
                                                                ...
                                                                GLint
                   iValue,iValue2; glGetIntegerv(GL_ATTRIB_STACK_DEPTH,&iValue);
                                                                glGetIntegerv(GL_MAX_ATTRIB_STACK_DEPTH,&iValue2);

                                                                OutputDebugString(va("iValue
                   %d, iValue2 %d\n",iValue,iValue2));

                                                                strJunk +=
                   va("iValue %d, iValue2 %d\n",iValue,iValue2);
                                                        }
                                                }
                */
                ...
                if (error && error != GL_STACK_OVERFLOW	/* fucking stupid ATI cards report this for no reason sometimes */ )
                {
                    ...
                    {
                        bKeepGettingErrors = GetYesNo(
                            va("Error: TextureList_ReMip()::glGetError = "
                               "%d\n\nContinue receiving GL errors here?",
                               error));
         ...
         ++it)
    {
        ...
        if (!pTexture->bIsDefault)
        {
            ...
            if (bRefresh)
            {
                ...
                pTexture->bFTValid = GetFileTime(
                    va("%s%s", pTexture->sName, pTexture->sExt), pTexture->ft);
                ...
                if (pTexture->pPixels) // file loaded ok?
                {
                    ...
                    if (error)
                    {
                        assert(0);
                        ErrorBox(
                            va("TextureList_Refresh::glGetError = %d", error));
                        return;
                    }
                }
                else
...
//
// Note: repeatedly asking for a missing textures will still disk-search at the
// moment, but who cares?
//
static TextureHandle_t Texture_Load_Actual(const char* psLocalTexturePath)
{
    ...
    strncpy(
        Texture.sExt,
        g_sImageExtension,
        sizeof(Texture.sExt)); // Filename_ExtOnly(psLocalTexturePath));
    Texture.sName[sizeof(Texture.sName) - 1] = '\0';
    Texture.sExt[sizeof(Texture.sExt) - 1] = '\0';
    Texture.bFTValid =
        GetFileTime(va("%s%s", Texture.sName, Texture.sExt), Texture.ft);

    glGenTextures(
        1, &Texture.gluiDesiredBind); // in case we fail, so we can still do a
                                      // refresh without having a zero bind
    ...
    // file not found, or bad format etc.... cache it anyway so it won't keep
    // reporting the error...
    //
    TextureHandle_t thNewHandle = TextureList_GetNewHandle();
    TheTextures[thNewHandle] = Texture;
...
TextureHandle_t
Texture_Load(const char* psLocalTexturePath, bool bInhibitStatus /* = false */)
{
    if (!bInhibitStatus)
    {
